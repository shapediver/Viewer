...
export class GeometryLoader implements ILoader {
    ...
    public loadGeometry(primitive: PrimitiveData): THREE.BufferGeometry {
        ...
        for (let attributeId in primitive.attributes) {
            ...
            if (attributeId === 'NORMAL')
                this.checkNormals(primitive, attributeId, geometry);
    ...
    private checkNormals(primitive: PrimitiveData, attributeId: string, geometry: THREE.BufferGeometry) {
        let blnNormalsOk = false;
        for (let index = 0; index < 10; ++index) {
            if (Math.abs(primitive.attributes[attributeId].array[index * 3]) > 0.001) {
                blnNormalsOk = true;
                break;
            }
            if (
                Math.abs(primitive.attributes[attributeId].array[index * 3 + 1]) > 0.001
            ) {
                blnNormalsOk = true;
                break;
            }
            if (
                Math.abs(primitive.attributes[attributeId].array[index * 3 + 2]) > 0.001
            ) {
                blnNormalsOk = true;
                break;
            }

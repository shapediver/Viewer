...
for (let c = 0; c < allCapabilities.length; c++) {
	const capabilities = Object.assign(
		{
			name: "mobile_tests",
			build: require("../../../api/default/package.json").version,
		},
		allCapabilities[c],
	);
	let name =
		"mobile_tests/" +
		((allCapabilities[c] as DesktopCapabilities).os
			? (<DesktopCapabilities>capabilities).os +
				"_" +
				(<DesktopCapabilities>capabilities).os_version +
				"_" +
				(<DesktopCapabilities>capabilities).browserName +
				"_" +
				(<DesktopCapabilities>capabilities).browser_version
			: (<MobileCapabilities>capabilities).device +
				"_" +
				(<MobileCapabilities>capabilities).os_version);
	...
	describe("device testing", () => {
		beforeAll(async () => {
			driver = await new webdriver.Builder()
				.usingServer(
					"http://alexanderschiftn1:csj6VCzMwzBYyRecsbm2@hub-cloud.browserstack.com/wd/hub",
				)

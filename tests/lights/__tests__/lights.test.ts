...
describe("device testing", () => {
	...
	test(name + "_default", async () => {
		const r: any = await driver.executeAsyncScript(
			async (ticket: string, cb: any) => {
	...
	test(name + "_disable", async () => {
		...
		const r2: any = await driver.executeAsyncScript(
			async (ticket: string, cb: any) => {
				const SDV: typeof ShapeDiverViewer = (<any>window).SDV;
				let viewer = await SDV.createViewport({
					id: "myViewer",

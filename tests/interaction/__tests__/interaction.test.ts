...
for(let c = 0; c < allCapabilities.length; c++) {
    ...
    const capabilities = Object.assign({ 'name': 'interaction_tests', 'build': require('../../../api/api/package.json').version }, allCapabilities[c]);

    if(process.env.PORT !== 'browserstack') {
        name = 'interaction_tests';
        c = allCapabilities.length;
    } else {
        name = 'interaction_tests/' + ((allCapabilities[c] as DesktopCapabilities).os ? 

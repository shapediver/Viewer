...
export const convertFromPrevious = (
	...
	for (const key in oldSettings.camera.cameras) {
		if (oldSettings.camera.cameras[key].type === "perspective") {
			const perspectiveCamera = oldSettings.camera.cameras[
				key
			] as IPerspectiveCameraSettings;

			settings.camera.cameras[key] = {
				autoAdjust: perspectiveCamera.autoAdjust,
				cameraMovementDuration:
					perspectiveCamera.cameraMovementDuration,
				controls: {
					autoRotationSpeed:
						perspectiveCamera.controls.autoRotationSpeed,
					damping: perspectiveCamera.controls.damping,
					enableAutoRotation:
						perspectiveCamera.controls.enableAutoRotation,
					...
					movementSmoothness:
						perspectiveCamera.controls.movementSmoothness,
				...
				revertAtMouseUpDuration:
					perspectiveCamera.revertAtMouseUpDuration,
			...
			const orthographicCamera = oldSettings.camera.cameras[
				key
			] as IPerspectiveCameraSettings;
			...
			settings.camera.cameras[key] = {
				...
				cameraMovementDuration:
					orthographicCamera.cameraMovementDuration,
				controls: {
					...
					movementSmoothness:
						orthographicCamera.controls.movementSmoothness,
				...
				revertAtMouseUpDuration:
					orthographicCamera.revertAtMouseUpDuration,
				sceneRotation: {x: 0, y: 0}, // This property did not exist in the previous version
...
export const convertToPrevious = (
	...
	for (const key in newSettings.camera.cameras) {
		if (newSettings.camera.cameras[key].type === "perspective") {
			const perspectiveCamera = newSettings.camera.cameras[
				key
			] as IPerspectiveCameraSettings;
			...
			const orthographicCamera = newSettings.camera.cameras[
				key
			] as IPerspectiveCameraSettings;

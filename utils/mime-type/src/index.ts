...
export const guessMimeTypeFromFilename = (filename: string): string[] => {
	...
	return extensionToMimeTypeMapping[
		extension as keyof typeof extensionToMimeTypeMapping
	];
};
...
export const mapMimeTypeToFileEndings = (mimeTypes: string[]): string[] => {
	...
	for (let i = 0; i < types.length; i++) {
		const fileEnding = Object.keys(extensionToMimeTypeMapping).find((key) =>
			extensionToMimeTypeMapping[key].includes(types[i]),
		);
...
export const extendMimeTypes = (mimeTypes: string[]): string[] => {
	...
	endings.forEach(
		(e: string) => (types = types.concat(guessMimeTypeFromFilename(e))),
	);

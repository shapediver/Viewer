...
export class MaterialEngine {
    ...
    public async loadMapWithProperties(texture: ITexture): Promise<MapData | null> {
        ...
        return new MapData(image, wrapS, wrapT, TEXTURE_FILTERING.LINEAR_MIPMAP_LINEAR, TEXTURE_FILTERING.LINEAR, center, this._converter.toHexColor(color), offset, repeat, texture.rotation || 0);
    }
    ...
    public async loadMaterialV3(data: IMaterialContentDataV3): Promise<MaterialStandardData> {
        ...
        if (data.color)
            material.color = this._converter.toHexColor(data.color);

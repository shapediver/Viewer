	...
	public async loadContent(content: ResOutputContent): Promise<ITreeNode> {
		try {
			...
			if (content.format === "tag2d") {
				...
				data.forEach((element: ITag2D) => {
					...
					if (!element.location || !element.text) {
						this._logger.warn(
							"HTMLElementAnchorEngine.load: One of the specified Tag2D elements did not have all necessary properties.",
						);
						return;
					}
					const cleanedText = this._inputValidator.sanitize(
						element.text,
					);
					node.data.push(
				...
				data.forEach((element: IAnchor) => {
					if (!element.location || !element.data) {
						this._logger.warn(
							"HTMLElementAnchorEngine.load: One of the specified Anchor elements did not have all necessary properties.",
						);
						return;
					}
					...
					if (element.intersectionTarget) {
						if (
							typeof element.intersectionTarget === "string" ||
							Array.isArray(element.intersectionTarget)
						) {
							intersectionTarget = element.intersectionTarget;
						} else if (
							element.intersectionTarget.min &&
							element.intersectionTarget.max
						) {
							intersectionTarget = new Box(
								this._converter.toVec3(
									element.intersectionTarget.min,
								),
								this._converter.toVec3(
									element.intersectionTarget.max,
					...
					if (!element.format || element.format === "text") {
						if (!(<IAnchorDataText>element.data).text) {
							this._logger.warn(
								"HTMLElementAnchorEngine.load: The text property for an Anchor element is missing.",
							);
							return;
						}
						...
						node.data.push(
							new HTMLElementAnchorTextData({
								location: this._converter.toVec3(
									element.location,
								),
								data: {
						...
						if (
							!(<IAnchorDataImage>element.data).src ||
							!(<IAnchorDataImage>element.data).width ||
							!(<IAnchorDataImage>element.data).height ||
							!(<IAnchorDataImage>element.data).alt
						) {
							this._logger.warn(
								"HTMLElementAnchorEngine.load: One of the specified Anchor elements did not have all necessary properties.",
							);
						...
						node.data.push(
							new HTMLElementAnchorImageData({
								location: this._converter.toVec3(
									element.location,
								),
								data: {
									alt: imageData.alt,
									height:
										typeof imageData.height === "string"
											? +imageData.height
											: imageData.height,
									width:
										typeof imageData.width === "string"
											? +imageData.width
											: imageData.width,
									src: imageData.src,
					...
					this._logger.warn(
						`HTMLElementAnchorEngine.load: The Anchor does not have a recognized format: ${element.format}`,
					);
				});
			}
			return node;
			...
			throw new ShapeDiverViewerDataProcessingError(
				"HTMLElementAnchorEngine.load: Loading of anchors failed.",
			);
		}
	}

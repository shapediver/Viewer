...
import {type Color, type ITag3D} from "@shapediver/viewer.shared.types";
	...
	public async loadContent(content: ResOutputContent): Promise<ITreeNode> {
		...
		if (!content)
			throw new ShapeDiverViewerDataProcessingError(
				"Tag3dEngine.loadContent: Invalid content was provided to tag3d engine.",
			);
				...
				for (const dictionary of dictionaries) {
					for (const key in dictionary) {
						if (textTagDictionary[key] === undefined) {
							...
							textTagDictionary[key].transformations.push(
								...dictionary[key].transformations,
							);
							textTagDictionary[key].colors.push(
								...dictionary[key].colors,
							);
						}
					}
				}
	...
	> {
		...
		characterGlb.traverseData((data) => {
			if (data instanceof CustomData) {
				if (data.data.font) fontInfo = data.data.font;
			}
		});
		...
		const characterHeight =
			((fontInfo.ascender - fontInfo.descender) / fontInfo.resolution) *
			scaledFontSize;
			...
			for (let j = 0; j < characters.length; ++j) {
					...
					if (typeof Tag3dEngine._fontInfo !== "string") {
						...
						if (!Tag3dEngine._nonAsciiGlb) {
							...
							Tag3dEngine._nonAsciiGlb =
								await geometryEngine.loadContent(
									{
				...
				if (nodesWithName.length !== 0) {
					const transformationMatrix = mat4.fromTranslation(
						mat4.create(),
						vec3.fromValues(lineWidth, lineHeight, 0),
					);
					...
					if (characterDictionary[char] === undefined) {
						...
						characterDictionary[char].transformations.push(
					...
					if (char !== " ")
						this._logger.warn(
							`Tag3dEngine.loadContent: Character ${char} not found in font.`,
						);
				}
			}
		...
		const characterHeightWithoutDescender =
			(fontInfo.ascender / fontInfo.resolution) * scaledFontSize;
		const characterDescender =
			(fontInfo.descender / fontInfo.resolution) * scaledFontSize;

		// some magic offsets to make the text appear correctly
		const initialMagicOffset = 0.1;
		const magicOffsetTop =
			initialMagicOffset - 0.06 * (charactersPerLine.length - 1);
		const top =
			-maxLineHeight +
		...
		const fontSizeVec3 = vec3.fromValues(
			scaledFontSize,
			scaledFontSize,
			scaledFontSize,
		);
		vec3.divide(tagJustTranslation, tagJustTranslation, fontSizeVec3);
		const tagJustTranslationMatrix = mat4.fromTranslation(
			mat4.create(),
			tagJustTranslation,
		);
		...
		if (tag3dInfo.location.xAxis !== undefined) {
			mat4.set(
				rotationMatrix,
				tag3dInfo.location.xAxis.X,
				tag3dInfo.location.xAxis.Y,
				tag3dInfo.location.xAxis.Z,
				0,
				tag3dInfo.location.yAxis.X,
				tag3dInfo.location.yAxis.Y,
				tag3dInfo.location.yAxis.Z,
				0,
				tag3dInfo.location.normal.X,
				tag3dInfo.location.normal.Y,
				tag3dInfo.location.normal.Z,
				0,
				0,
				0,
				0,
			...
			mat4.fromTranslation(
				translationMatrix,
				vec3.fromValues(
					tag3dInfo.location.origin.X,
					tag3dInfo.location.origin.Y,
					tag3dInfo.location.origin.Z,
				),
			);
		}

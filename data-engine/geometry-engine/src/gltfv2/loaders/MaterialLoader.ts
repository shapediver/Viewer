...
export class MaterialLoader {
	...
	public getMaterial(materialId: number): IMaterialAbstractData {
		if (!this._content.materials)
			throw new Error(
				"MaterialLoader.getMaterial: Materials not available.",
			);
		if (!this._content.materials[materialId])
			throw new Error(
				"MaterialLoader.getMaterial: Material not available.",
			);
	...
	public async load(): Promise<void> {
		...
		for (let i = 0; i < this._content.materials.length; i++) {
			const materialId = i;
			const material: IGLTF_v2_Material =
				this._content.materials[materialId];
			...
			if (
				materialExtensions[GLTF_EXTENSIONS.SHAPEDIVER_MATERIALS_PRESET]
			) {
				const materialPreset: ISHAPEDIVER_materials_preset =
					materialExtensions[
						GLTF_EXTENSIONS.SHAPEDIVER_MATERIALS_PRESET
					];
				promises.push(
			...
			if (
				materialExtensions[
					GLTF_EXTENSIONS.KHR_MATERIALS_PBRSPECULARGLOSSINESS
				]
			) {
				const pbrSpecularGlossiness: IGLTF_v2_Material_KHR_materials_pbrSpecularGlossiness =
					materialExtensions[
						GLTF_EXTENSIONS.KHR_MATERIALS_PBRSPECULARGLOSSINESS
					];
				const specularGlossinessMaterialDataProperties: IMaterialSpecularGlossinessDataProperties =
				...
				if (pbrSpecularGlossiness.diffuseTexture !== undefined) {
					const diffuseTextureOptions =
						pbrSpecularGlossiness.diffuseTexture.extensions &&
						pbrSpecularGlossiness.diffuseTexture.extensions[
							GLTF_EXTENSIONS.KHR_TEXTURE_TRANSFORM
						]
							? pbrSpecularGlossiness.diffuseTexture.extensions[
									GLTF_EXTENSIONS.KHR_TEXTURE_TRANSFORM
								]
					...
					if (
						...
						diffuseTextureOptions.texCoord =
							pbrSpecularGlossiness.diffuseTexture.texCoord;
					specularGlossinessMaterialDataProperties.map = this.loadMap(
				...
				if (
					...
					const specularGlossinessTextureOptions =
						pbrSpecularGlossiness.specularGlossinessTexture
							.extensions &&
						pbrSpecularGlossiness.specularGlossinessTexture
							.extensions[GLTF_EXTENSIONS.KHR_TEXTURE_TRANSFORM]
							? pbrSpecularGlossiness.specularGlossinessTexture
									.extensions[
									GLTF_EXTENSIONS.KHR_TEXTURE_TRANSFORM
								]
					...
					specularGlossinessMaterialDataProperties.specularGlossinessMap =
						this.loadMap(
							pbrSpecularGlossiness.specularGlossinessTexture
								.index,
							specularGlossinessTextureOptions,
						);
				}
			} else if (
				materialExtensions[GLTF_EXTENSIONS.KHR_MATERIALS_UNLIT]
			) {
				const unlitMaterialDataProperties: IMaterialUnlitDataProperties =
					materialDataProperties;
				...
				if (material.pbrMetallicRoughness !== undefined) {
					...
					if (
						...
						unlitMaterialDataProperties.map = this.loadMap(
							material.pbrMetallicRoughness.baseColorTexture
								.index,
							baseColorTextureOptions,
						);
					}
				}
			} else {
				const standardMaterialDataProperties: IMaterialStandardDataProperties =
					materialDataProperties;
				if (material.pbrMetallicRoughness !== undefined) {
					...
					if (
						...
						if (
							...
							baseColorTextureOptions.texCoord =
								material.pbrMetallicRoughness.baseColorTexture.texCoord;
						standardMaterialDataProperties.map = this.loadMap(
							material.pbrMetallicRoughness.baseColorTexture
								.index,
							baseColorTextureOptions,
						);
					...
					if (
						...
						const metallicRoughnessTextureOptions =
							material.pbrMetallicRoughness
						...
						standardMaterialDataProperties.metalnessRoughnessMap =
							this.loadMap(
								...
								metallicRoughnessTextureOptions,
							);
					}
				}
			}
			...
			if (material.normalTexture !== undefined) {
				...
				materialDataProperties.normalMap = this.loadMap(
			...
			if (material.occlusionTexture !== undefined) {
				...
				materialDataProperties.aoMap = this.loadMap(
					material.occlusionTexture.index,
					occlusionTextureOptions,
				);
			...
			if (material.emissiveTexture !== undefined) {
				...
				materialDataProperties.emissiveMap = this.loadMap(
			...
			if (material.alphaMode !== undefined) {
				materialDataProperties.alphaMode =
					material.alphaMode.toLowerCase() === MATERIAL_ALPHA.MASK
						? MATERIAL_ALPHA.MASK
						: material.alphaMode.toLowerCase() ===
							  MATERIAL_ALPHA.BLEND
							? MATERIAL_ALPHA.BLEND
							: MATERIAL_ALPHA.OPAQUE;
			...
			if (
				...
				const specularGlossinessMaterialDataProperties: IMaterialSpecularGlossinessDataProperties =
					materialDataProperties;
				const materialData = new MaterialSpecularGlossinessData(
				...
				const unlitMaterialDataProperties: IMaterialUnlitDataProperties =
					materialDataProperties;
				const materialData = new MaterialUnlitData(
			...
			if (materialExtensions[GLTF_EXTENSIONS.KHR_MATERIALS_CLEARCOAT]) {
				const clearcoatExtension =
					materialExtensions[GLTF_EXTENSIONS.KHR_MATERIALS_CLEARCOAT];
				...
				if (clearcoatExtension.clearcoatTexture !== undefined) {
					const clearcoatTextureOptions =
						clearcoatExtension.clearcoatTexture.extensions &&
						clearcoatExtension.clearcoatTexture.extensions[
							GLTF_EXTENSIONS.KHR_TEXTURE_TRANSFORM
						]
							? clearcoatExtension.clearcoatTexture.extensions[
					...
					if (
						...
						clearcoatTextureOptions.texCoord =
							clearcoatExtension.clearcoatTexture.texCoord;
					standardMaterialDataProperties.clearcoatMap = this.loadMap(
						clearcoatExtension.clearcoatTexture.index,
						clearcoatTextureOptions,
					);
				}
				...
				if (
					clearcoatExtension.clearcoatRoughnessTexture !== undefined
				) {
					const clearcoatRoughnessTextureOptions =
						clearcoatExtension.clearcoatRoughnessTexture
							.extensions &&
						clearcoatExtension.clearcoatRoughnessTexture.extensions[
							GLTF_EXTENSIONS.KHR_TEXTURE_TRANSFORM
						]
							? clearcoatExtension.clearcoatRoughnessTexture
									.extensions[
					...
					standardMaterialDataProperties.clearcoatRoughnessMap =
						this.loadMap(
							clearcoatExtension.clearcoatRoughnessTexture.index,
							clearcoatRoughnessTextureOptions,
						);
				}

				if (clearcoatExtension.clearcoatNormalTexture !== undefined) {
					const clearcoatNormalTextureOptions =
						clearcoatExtension.clearcoatNormalTexture.extensions &&
						clearcoatExtension.clearcoatNormalTexture.extensions[
							GLTF_EXTENSIONS.KHR_TEXTURE_TRANSFORM
						]
					...
					standardMaterialDataProperties.clearcoatNormalMap =
						this.loadMap(
							clearcoatExtension.clearcoatNormalTexture.index,
							clearcoatNormalTextureOptions,
						);
				}
			}
			...
			if (materialExtensions[GLTF_EXTENSIONS.KHR_MATERIALS_IOR]) {
				const iorExtension =
					materialExtensions[GLTF_EXTENSIONS.KHR_MATERIALS_IOR];
			...
			if (
				materialExtensions[GLTF_EXTENSIONS.KHR_MATERIALS_TRANSMISSION]
			) {
				const transmissionExtension =
					materialExtensions[
						GLTF_EXTENSIONS.KHR_MATERIALS_TRANSMISSION
					];
				...
				if (transmissionExtension.transmissionTexture !== undefined) {
					const transmissionTextureOptions =
						transmissionExtension.transmissionTexture.extensions &&
						transmissionExtension.transmissionTexture.extensions[
							GLTF_EXTENSIONS.KHR_TEXTURE_TRANSFORM
						]
					...
					standardMaterialDataProperties.transmissionMap =
						this.loadMap(
							transmissionExtension.transmissionTexture.index,
							transmissionTextureOptions,
						);
				}
			}

			if (materialExtensions[GLTF_EXTENSIONS.KHR_MATERIALS_VOLUME]) {
				const volumeExtension =
					materialExtensions[GLTF_EXTENSIONS.KHR_MATERIALS_VOLUME];
				...
				if (volumeExtension.thicknessTexture !== undefined) {
					const thicknessTextureOptions =
						volumeExtension.thicknessTexture.extensions &&
						volumeExtension.thicknessTexture.extensions[
							GLTF_EXTENSIONS.KHR_TEXTURE_TRANSFORM
						]
							? volumeExtension.thicknessTexture.extensions[
					...
					standardMaterialDataProperties.thicknessMap = this.loadMap(
						volumeExtension.thicknessTexture.index,
						thicknessTextureOptions,
					);
				}
			...
			if (materialExtensions[GLTF_EXTENSIONS.KHR_MATERIALS_SHEEN]) {
				const sheenExtension =
					materialExtensions[GLTF_EXTENSIONS.KHR_MATERIALS_SHEEN];
				...
				if (sheenExtension.sheenColorTexture !== undefined) {
					const sheenColorTextureOptions =
					...
					standardMaterialDataProperties.sheenColorMap = this.loadMap(
						sheenExtension.sheenColorTexture.index,
						sheenColorTextureOptions,
					);
				}

				if (sheenExtension.sheenRoughnessTexture !== undefined) {
					const sheenRoughnessTextureOptions =
						sheenExtension.sheenRoughnessTexture.extensions &&
						sheenExtension.sheenRoughnessTexture.extensions[
							GLTF_EXTENSIONS.KHR_TEXTURE_TRANSFORM
						]
							? sheenExtension.sheenRoughnessTexture.extensions[
					...
					standardMaterialDataProperties.sheenRoughnessMap =
						this.loadMap(
							sheenExtension.sheenRoughnessTexture.index,
							sheenRoughnessTextureOptions,
						);
				}
			}

			if (materialExtensions[GLTF_EXTENSIONS.KHR_MATERIALS_SPECULAR]) {
				const specularExtension =
					materialExtensions[GLTF_EXTENSIONS.KHR_MATERIALS_SPECULAR];
				...
				if (specularExtension.specularColorTexture !== undefined) {
					const specularColorTextureOptions =
						specularExtension.specularColorTexture.extensions &&
						specularExtension.specularColorTexture.extensions[
							GLTF_EXTENSIONS.KHR_TEXTURE_TRANSFORM
						]
							? specularExtension.specularColorTexture.extensions[
					...
					standardMaterialDataProperties.specularColorMap =
						this.loadMap(
							specularExtension.specularColorTexture.index,
							specularColorTextureOptions,
						);
				}

				if (specularExtension.specularTexture !== undefined) {
					const specularTextureOptions =
						specularExtension.specularTexture.extensions &&
						specularExtension.specularTexture.extensions[
							GLTF_EXTENSIONS.KHR_TEXTURE_TRANSFORM
						]
							? specularExtension.specularTexture.extensions[
							...
							: {};
					if (
						specularExtension.specularTexture.texCoord !== undefined
					)
					...
					standardMaterialDataProperties.specularIntensityMap =
						this.loadMap(
							specularExtension.specularTexture.index,
		...
		return new MapData(loadedTexture.image, {
			...
			repeat:
				properties && properties.scale
					? vec2.fromValues(properties.scale[0], properties.scale[1])
					: vec2.fromValues(1, 1),
			rotation:
				properties && properties.rotation !== undefined
					? properties.rotation
					: 0,

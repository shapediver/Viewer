...
export class SDGTFLoader {
	...
	public async load(
		...
		if (gltfLength < binaryGeometry.byteLength) {
			const headerDataView = new DataView(
				binaryGeometry,
				gltfLength,
				this.BINARY_EXTENSION_HEADER_LENGTH + 1,
			);
			const header = {
				magic:
	...
	private async loadAccessor(accessorName: string): Promise<AttributeData> {
		...
		return new AttributeData(
			new ArrayType(bufferView, byteOffset, itemSize * accessor.count),
			itemSize,
			itemBytes,
			byteOffset,
			elementBytes,
	...
	private async loadArcs(): Promise<ITreeNode> {
		...
		for (let i = 0; i < count; ++i) {
			...
			const index = i * 12;
			const arcCenter = vec3.fromValues(
				data.array[index + 0],
				data.array[index + 1],
				data.array[index + 2],
			);
			const arcXAxis = vec3.fromValues(
				data.array[index + 3],
				data.array[index + 4],
				data.array[index + 5],
			);
			const arcYAxis = vec3.fromValues(
				data.array[index + 6],
				data.array[index + 7],
				data.array[index + 8],
			);
			...
			const numberOfPoints = Math.max(
				3,
				Math.round(50 * (((arcMaxAngle - arcMinAngle) / 2) * Math.PI)),
			);
			...
			attributes["POSITION"] = new AttributeData(
				array,
				3,
				0,
				0,
			...
			const geometry = new GeometryData(
				new PrimitiveData(attributes, null),
			...
			singleArcNode.addTransformation({
				...
				matrix: mat4.translate(
					mat4.create(),
					mat4.create(),
					vec3.fromValues(arcCenter[0], arcCenter[1], arcCenter[2]),
				),
			});

			const arcRotationMatrix = mat4.transpose(
				mat4.create(),
				mat4.fromValues(
					arcXAxis[0],
					arcYAxis[0],
	...
	private async loadBeziercurve(beziercurveName: string): Promise<ITreeNode> {
		...
		const controlPointsData = await this.loadAccessor(
			beziercurve.attributes["CONTROLPOINTS"],
		); // vec3
		const controlPoints: vec4[] = [];
		for (let i = 0; i < controlPointsData.array.length; i += 3)
			controlPoints.push(
				vec4.fromValues(
					controlPointsData.array[i],
					controlPointsData.array[i + 1],
					controlPointsData.array[i + 2],
					1,
				),
			);

		const knotsData = await this.loadAccessor(
			beziercurve.attributes["KNOTS"],
		); // scalar
		...
		const getPointOnBezierCurve = (t: number): void => {
			...
			if (hpoint[3] !== 1.0) {
				...
				hpoint = vec4.divide(
					vec4.create(),
					hpoint,
					vec4.fromValues(hpoint[3], hpoint[3], hpoint[3], hpoint[3]),
				);
			}
		...
		const numberOfPoints = Math.min(
			100,
			Math.max(25, Math.floor(distance / 0.1)),
		);
		for (let d = 0; d <= numberOfPoints; d++)
	...
	private async loadCircles(): Promise<ITreeNode> {
		...
		for (let i = 0; i < count; i++) {
			...
			const index = i * 10;
			const circleCenter = vec3.fromValues(
				data.array[index + 0],
				data.array[index + 1],
				data.array[index + 2],
			);
			const circleXAxis = vec3.fromValues(
				data.array[index + 3],
				data.array[index + 4],
				data.array[index + 5],
			);
			const circleYAxis = vec3.fromValues(
				data.array[index + 6],
				data.array[index + 7],
				data.array[index + 8],
			);
			const circleRadius = data.array[index + 9];
			const circleZAxis = vec3.cross(
				vec3.create(),
				circleXAxis,
				circleYAxis,
			);
			...
			if (circleRadius <= 0) {
				this._logger.warn(
					"SDGTFLoader.loadCircles: Circle radius is <= 0.",
				);
				continue;
			}
			...
			const circleRotationMatrix = mat4.transpose(
				mat4.create(),
				mat4.fromValues(
					circleXAxis[0],
					circleYAxis[0],
	...
	private async loadCylinders(): Promise<ITreeNode> {
		...
		for (let i = 0; i < count; i++) {
			...
			const index = i * 7;
			const cylinderTop = vec3.fromValues(
				data.array[index + 0],
				data.array[index + 1],
				data.array[index + 2],
			);
			const cylinderBottom = vec3.fromValues(
				data.array[index + 3],
				data.array[index + 4],
				data.array[index + 5],
			);
			const cylinderRadius = data.array[index + 6];
			const cylinderAxis = vec3.sub(
				vec3.create(),
				cylinderTop,
				cylinderBottom,
			);
			const dotX = Math.abs(
				vec3.dot(cylinderAxis, vec3.fromValues(1, 0, 0)),
			);
			const dotY = Math.abs(
				vec3.dot(cylinderAxis, vec3.fromValues(0, 1, 0)),
			);

			let cylinderXAxis: vec3;
			if (dotX < dotY) {
				cylinderXAxis = vec3.cross(
					vec3.create(),
					cylinderAxis,
			...
			const cylinderYAxis = vec3.cross(
				vec3.create(),
				cylinderAxis,
				cylinderXAxis,
			);
			...
			if (cylinderRadius <= 0) {
				this._logger.warn(
					"SDGTFLoader.loadCylinders: Cylinder radius is <= 0.",
				);
				continue;
			}
			...
			for (let y = 0; y <= heightSegments; y++) {
				...
				for (let x = 0; x <= radialSegments; x++) {
					...
					vec3.normalize(
						normal,
						vec3.fromValues(sinTheta, slope, cosTheta),
					);
			...
			attributes["NORMAL"] = new AttributeData(
			...
			attributes["TEXCOORD_0"] = new AttributeData(
			...
			const geometry = new GeometryData(
				new PrimitiveData(
					attributes,
					new AttributeData(
						this.convertToIndicesArray(indices),
						1,
						0,
						0,
						0,
			...
			const cylinderRotationMatrix = mat4.transpose(
				mat4.create(),
				mat4.fromValues(
					cylinderXAxis[0],
					cylinderYAxis[0],
			...
			singleCylinderNode.addTransformation({
				...
				matrix: mat4.translate(
					...
					vec3.fromValues(
						0,
						0,
						0.5 * vec3.distance(cylinderTop, cylinderBottom),
					),
				),
			});
	...
	private async loadSpheres(): Promise<ITreeNode> {
		...
		for (let i = 0; i < count; i++) {
			...
			const index = i * 4;
			const sphereTranslation = vec3.fromValues(
				data.array[index + 0],
				data.array[index + 1],
				data.array[index + 2],
			);
			...
			if (sphereRadius <= 0) {
				this._logger.warn(
					"SDGTFLoader.loadSpheres: Sphere radius is <= 0.",
				);
				continue;
			}
	...
	private async loadSurfacepatch(
		surfacepatchName: string,
	): Promise<ITreeNode> {
		...
		const controlPointsData = await this.loadAccessor(
			surfacepatch.attributes["CONTROLPOINTS"],
		); // vec3
		const controlPoints: vec4[][] = [];
		let pointCount = 0;
		for (let u = 0; u < controlPointCountU; u++) {
			...
			for (let v = 0; v < controlPointCountV; v++) {
				innerArray.push(
					vec4.fromValues(
						controlPointsData.array[pointCount * 3],
						controlPointsData.array[pointCount * 3 + 1],
						controlPointsData.array[pointCount * 3 + 2],
						1,
					),
				);
				pointCount++;
		...
		const knotsUData = await this.loadAccessor(
			surfacepatch.attributes["KNOTSU"],
		); // scalar
		...
		const knotsVData = await this.loadAccessor(
			surfacepatch.attributes["KNOTSV"],
		); // scalar
		...
		const calcSurfacePoint = (u: number, v: number): vec3 => {
			...
			for (let l = 0; l <= degreeV; ++l) {
				...
				for (let k = 0; k <= degreeU; ++k) {
					const point = vec4.clone(
						controlPoints[uspan - degreeU + k][vspan - degreeV + l],
					);
					const w = point[3];
					point[0] *= w;
					point[1] *= w;
					...
					vec4.add(
						...
						vec4.multiply(
							vec4.create(),
							point,
							vec4.fromValues(Nu[k], Nu[k], Nu[k], Nu[k]),
						),
					);
				}
			}
			...
			for (let l = 0; l <= degreeV; ++l) {
				vec4.add(
					...
					vec4.multiply(
						vec4.create(),
						temp[l],
						vec4.fromValues(Nv[l], Nv[l], Nv[l], Nv[l]),
					),
				);
			}
	...
	private async loadScene(): Promise<ITreeNode> {
		...
		// arcs
		if (this._content.arcs) sceneNode.addChild(await this.loadArcs());
		...
		// circles
		if (this._content.circles) sceneNode.addChild(await this.loadCircles());
		...
		// spheres
		if (this._content.spheres) sceneNode.addChild(await this.loadSpheres());

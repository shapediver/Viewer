...
export class SDGTFLoader {
    ...
    public async load(binaryGeometry: ArrayBuffer, gltfLength: number): Promise<TreeNode> {
        if (gltfLength < binaryGeometry.byteLength) {
            ...
            if (header.magic != 'sdgTF') {
                ...
                throw this._logger.handleError(LOGGINGTOPIC.DATA_PROCESSING, `SDGTFLoader.load`, error);
            }
        ...
        try {
            ...
            throw this._logger.handleError(LOGGINGTOPIC.DATA_PROCESSING, `SDGTFLoader.load`, e);
        }
    }
    ...
    private async loadArcs(): Promise<TreeNode> {
        ...
        for (let i = 0; i < count; ++i) {
            ...
            if (arcRadius <= 0) {
                this._logger.warn(LOGGINGTOPIC.DATA_PROCESSING, 'SDGTFLoader.loadArcs: Arc radius is <= 0.');
                continue;
            }
            const points: number[] = [];
            ...
            const geometry = new GeometryData(new PrimitiveData(attributes, PRIMITIVE_MODE.LINE_STRIP, null, new MaterialStandardData({ color: '#d3d3d3', roughness: 1, metalness: 0 })));
    ...
    private async loadCircles(): Promise<TreeNode> {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (circleRadius <= 0) {
                this._logger.warn(LOGGINGTOPIC.DATA_PROCESSING, 'SDGTFLoader.loadCircles: Circle radius is <= 0.');
                continue;
            }
    ...
    private async loadCylinders(): Promise<TreeNode> {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (cylinderRadius <= 0) {
                this._logger.warn(LOGGINGTOPIC.DATA_PROCESSING, 'SDGTFLoader.loadCylinders: Cylinder radius is <= 0.');
                continue;
            }
            ...
            const geometry = new GeometryData(new PrimitiveData(attributes, PRIMITIVE_MODE.TRIANGLES, new AttributeData(this.convertToIndicesArray(indices), 1, 0, 0, 0, false, indices.length), new MaterialStandardData({ color: '#d3d3d3', roughness: 1, metalness: 0 })));
    ...
    private async loadSpheres(): Promise<TreeNode> {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (sphereRadius <= 0) {
                this._logger.warn(LOGGINGTOPIC.DATA_PROCESSING, 'SDGTFLoader.loadSpheres: Sphere radius is <= 0.');
                continue;
            }
    ...
    private async loadPoint(pointName: string): Promise<TreeNode> {
        ...
        const geometry = new GeometryData(new PrimitiveData(attributes, PRIMITIVE_MODE.POINTS, null, new MaterialStandardData({ color: '#d3d3d3', roughness: 1, metalness: 0 })));

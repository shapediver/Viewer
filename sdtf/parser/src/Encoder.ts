...
export class Encoder {
    ...
    public async encodeFromUriToArrayBuffer(uri: string): Promise<ArrayBuffer | null> {
        ...
        try {
            ...
            this._logger.error(`Was not able to get array buffer from uri.`, e, e.response && e.response.status ? e.response.status : null);
            return null;
        }
        ...
        if (axiosResponse.headers['content-type'] && axiosResponse.headers['content-type'] === 'model/vnd.sdtf') {
            ...
            this._logger.error('Non-binary SDTF encoding not implemented.');
            return null;
        }
    }
    ...
    public async encodeFromArrayBufferToJson(arrayBuffer: ArrayBuffer): Promise<{
        ...
        if (magic !== 'sdtf') {
            this._logger.error('Invalid data: sdtf magic wrong.');
            return null;
        } 
        const version = headerDataView.getUint32(4, true);
        if (version !== 1) {
            this._logger.error(`Invalid version: sdtf loader does not support version ${version}.`);
            return null;
        } 
        ...
        if (contentFormat !== 0) {
            this._logger.error('Content format is not Json (0), content invalid.');
            return null;
        }

...
export class FileParameter extends Parameter<File | Blob | string> {
    ...
    public async upload() {
        ...
        if (data.size === 0) {
            this.#logger.error('Error uploading parameter ' + this.id + ': file size is 0.');
            return;
        }
        ...
        try {
            ...
            this.#logger.error('Upload request failed.', e, e.response && e.response.status ? e.response.status : null);
        }
    }

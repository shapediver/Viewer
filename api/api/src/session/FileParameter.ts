...
export class FileParameter extends Parameter<File | Blob | string> {
    ...
    public async upload() {
        this.#logger.debugLow(`Parameter(${this.id}).upload: Uploading FileParameter with value ${this.value}.`);
        ...
        if (data.size === 0)
            this.#logger.errorMessage(`Parameter(${this.id}).upload: Error uploading FileParameter, file size was 0.`);

        this.#logger.info(`Parameter(${this.id}).upload: Uploading FileParameter.`);
        try {
            ...
            if (e.response && e.response.status) {
                throw this.#logger.httpError(`Parameter(${this.id}).upload: Upload failed.`, e, e.response.status, true);
            } else {
                throw this.#logger.error(`Parameter(${this.id}).upload: Upload failed.`, e, true);
            }
        }
    }

...
const updateViewports = (renderingEngines: {
	...
	for (const v in renderingEngines)
		if (!viewports[v]) viewports[v] = new ViewportApi(renderingEngines[v]);

	for (const v in viewports) {
		if (!renderingEngines[v]) delete viewports[v];
	}
};
...
export const createViewport = async (
	...
	logger.debug(
		`createSession: Creating and initializing session with properties ${JSON.stringify(copy)}.`,
	);
	...
	inputValidator.validateAndError(
		...
		"enum",
		false,
		Object.values(SESSION_SETTINGS_MODE),
	);
	inputValidator.validateAndError(
		"createViewport",
		prop.visibilitySessionIds,
		"array",
		false,
	);
	inputValidator.validateAndError(
	...
	inputValidator.validateAndError(
		"createViewport",
		branding.busyModeDisplay,
		"enum",
		false,
		Object.values(BUSY_MODE_DISPLAY),
	);
	inputValidator.validateAndError(
		"createViewport",
		branding.spinnerPositioning,
		"enum",
		false,
	...
	if (
		prop.sessionSettingsMode === SESSION_SETTINGS_MODE.MANUAL &&
		!prop.sessionSettingsId
	)
		throw new ShapeDiverViewerValidationError(
			"createViewport: Input could not be validated. sessionSettingsId has to point to a valid and created session when using SESSION_SETTINGS_MODE.MANUAL",
			prop.sessionSettingsId,
			"string",
		);

	const renderingEngine =
		await creationControlCenterViewport.createViewportEngine(prop);

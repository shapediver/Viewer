...
export class Sphere implements ISphere {
	...
	public applyMatrix(matrix: mat4): Sphere {
		...
		const scaleZSq =
			...
			matrix[9] * matrix[9] +
			matrix[10] * matrix[10];
		const maxScaleOnAxis = Math.sqrt(
	...
	public intersect(origin: vec3, direction: vec3): number | null {
		...
		const distanceToProjection = vec3.squaredDistance(
			rayToSphere,
			vec3.multiply(
				vec3.create(),
				direction,
				vec3.fromValues(projection, projection, projection),
			),
		);
		...
		if (distanceToProjection <= this.radius * this.radius) {
			...
			const distanceToIntersection = Math.sqrt(
				this.radius * this.radius - distanceToProjection,
			);

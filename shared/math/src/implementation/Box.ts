...
export class Box implements IBox {
	...
	public get boundingSphere(): ISphere {
		if (
			!(
				this._boundingSphereState.min[0] === this.min[0] &&
	...
	public applyMatrix(matrix: mat4): Box {
		...
		for (let i = 0, il = points.length; i < il; i++) {
			this.min = vec3.fromValues(
				Math.min(this.min[0], points[i][0]),
				Math.min(this.min[1], points[i][1]),
				Math.min(this.min[2], points[i][2]),
			);
			this.max = vec3.fromValues(
				Math.max(this.max[0], points[i][0]),
				Math.max(this.max[1], points[i][1]),
				Math.max(this.max[2], points[i][2]),
			);
		}
		return this;
	}

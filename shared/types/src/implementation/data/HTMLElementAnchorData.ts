...
export abstract class HTMLElementAnchorData
	...
	implements IHTMLElementAnchorData
{
	// #region Properties (9)
	...
	constructor(properties: {
		...
		this.#hideable =
			properties.hideable === undefined ? true : properties.hideable;
		this.#viewports = properties.viewports || [];
		this.#intersectionTarget = properties.intersectionTarget;

		this.internalHtmlElement = <HTMLDivElement>(
	...
	public createViewerHtmlElement(viewer: string): HTMLDivElement | null {
		if (this.#viewerHtmlElement[viewer])
			return this.#viewerHtmlElement[viewer];
		if (this.viewports.includes(viewer) || this.viewports.length === 0) {
			this.#viewerHtmlElement[viewer] = <HTMLDivElement>(
				this.internalHtmlElement.cloneNode(true)
			);
			this.create({
				anchor: this,
				parent: this.#viewerHtmlElement[viewer],
			});
			return this.#viewerHtmlElement[viewer];
		}
		return null;
	...
	public abstract create(properties: {
...
export class HTMLElementAnchorTextData extends HTMLElementAnchorData {
	...
	public create(properties: {
		...
		if (
			properties.anchor.data.textAlign &&

	...
	implements IAnimationData
{
	...
	public startAnimation() {
		...
		for (let i = 0; i < this.#tracks.length; i++) {
			...
			const idleTransformation = track.node.transformations.find(
				(t) => t.id === "gltf_matrix",
			);
			...
			switch (track.path) {
				case "scale":
					const idleTransformationScale =
						track.node.transformations.find(
							(t) => t.id === "gltf_matrix_scale",
						);
				...
				case "rotation":
					const idleTransformationRotation =
						track.node.transformations.find(
							(t) => t.id === "gltf_matrix_rotation",
						);
				...
				case "translation":
					const idleTransformationTranslation =
						track.node.transformations.find(
							(t) => t.id === "gltf_matrix_translation",
						);
	...
	public stopAnimation() {
		if (this.reset) {
			for (let i = 0; i < this.#tracks.length; i++) {
				...
				const prevAnimation = track.node.transformations.filter(
					(t) => t.id === id,
				);
				track.node.transformations = track.node.transformations.filter(
					(el) => {
				...
				if (track.previousMatrix) {
					if (track.previousMatrix.id === "gltf_matrix") {
						const transformation = track.node.transformations.find(
							(t) => t.id === "gltf_matrix",
						)!;
						transformation.matrix = track.previousMatrix.matrix;
						const translationTransformation =
							track.node.transformations.find(
								(t) => t.id === "gltf_matrix_translation",
							)!;
						translationTransformation.matrix = mat4.create();
						const rotationTransformation =
							track.node.transformations.find(
								(t) => t.id === "gltf_matrix_rotation",
							)!;
			...
			for (let i = 0; i < this.#tracks.length; i++) {
				...
				let translationTransformation = track.node.transformations.find(
					(t) => t.id === "gltf_matrix_translation",
				);
				...
				let rotationTransformation = track.node.transformations.find(
					(t) => t.id === "gltf_matrix_rotation",
				);
				...
				let scaleTransformation = track.node.transformations.find(
					(t) => t.id === "gltf_matrix_scale",
				);
				...
				if (track.path === "rotation") {
					...
					if (track.pivot) {
						pivotMatrix = mat4.fromTranslation(
							mat4.create(),
							vec3.fromValues(
								track.pivot[0],
								track.pivot[1],
						...
						pivotMatrixInverse = mat4.fromTranslation(
							mat4.create(),
							vec3.fromValues(
								-track.pivot[0],
								-track.pivot[1],
					...
					let quaternion: quat = quat.fromValues(
						track.values[j * 4 + 0],
						track.values[j * 4 + 1],
						track.values[j * 4 + 2],
						track.values[j * 4 + 3],
					...
					if (pivotMatrix && pivotMatrixInverse) {
						rotationTransformation.matrix = mat4.multiply(
							mat4.create(),
							mat4.multiply(
								mat4.create(),
								pivotMatrix,
					...
					let vector: vec3 = vec3.fromValues(
						track.values[j * 3 + 0],
						track.values[j * 3 + 1],
						track.values[j * 3 + 2],
					);
					translationTransformation.matrix = mat4.fromTranslation(
						mat4.create(),
						vector,
					);
					...
					scaleTransformation.matrix = mat4.fromScaling(
						mat4.create(),
						vector,
					);

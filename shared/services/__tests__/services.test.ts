...
describe("test", () => {
	...
	it("test 2", async () => {
		let count = 0;
		const token1 = instance.addListener(
			EVENTTYPE.CAMERA,
			(event: IEvent) => {
				count++;
			},
	...
	it("test 3", async () => {
		...
		const token2 = instance.addListener(
			EVENTTYPE.CAMERA,
			(event: IEvent) => {
				count--;
			},
	...
	it("test 7", async () => {
		...
		const token3 = instance.addListener(
			EVENTTYPE.CAMERA.CAMERA_MOVE,
			(event: IEvent) => {
				count++;
			},
		);
		const token4 = instance.addListener(
			EVENTTYPE.CAMERA.CAMERA_END,
			(event: IEvent) => {
				count++;
			},
		);
...
describe("uuid - test", () => {
	...
	test("validator - custom correct", async () => {
		expect(
			uuidGenerator.validate("bacdefbc-fceb-4cbe-acbd-abcdefdacbab"),
		).toBe(true);
		expect(
			uuidGenerator.validate("73464832-2342-4344-8565-567575755555"),
		).toBe(true);
		expect(
			uuidGenerator.validate("aaaaaaaa-aaaa-4aaa-aaaa-aaaaaaaaaaaa"),
		).toBe(true);
		expect(
			uuidGenerator.validate("00000000-0000-4000-8000-000000000000"),
		).toBe(true);
		expect(
			uuidGenerator.validate("ffffffff-ffff-4fff-bfff-ffffffffffff"),
		).toBe(true);
	});

	test("validator - custom faulty", async () => {
		expect(
			uuidGenerator.validate("bacdefbc-fceb-bcbe-acbd-abcdefdacbab"),
		).toBe(false);
		expect(
			uuidGenerator.validate("zacdefbc-fceb-fcbe-acbd-abcdefdacbab"),
		).toBe(false);
		expect(
			uuidGenerator.validate("z3464832-2342-2344-6565-567575755555"),
		).toBe(false);
		expect(
			uuidGenerator.validate("45gg54g4-fg47-kj77-j687-7oi7o7o7oou7"),
		).toBe(false);
		expect(uuidGenerator.validate("")).toBe(false);
		expect(
			uuidGenerator.validate("bacdefb-fceb-fcbe-acbd-abcdefdacbab"),
		).toBe(false);
		expect(
			uuidGenerator.validate("bacdefb2-fceb-fcbe-acbd-abcdefdacba"),
		).toBe(false);
		expect(uuidGenerator.validate("bacdefb2-fceb-acbd-abcdefdacbad")).toBe(
			false,
		);
	});
	...
	test("parser - stringify", async () => {
		const uuid1: string = uuidGenerator.create();
		const uuid1Converted: string = uuidGenerator.stringify(
			uuidGenerator.parse(uuid1),
		);

...
export class EventEngine {
	...
	private constructor() {
		...
		for (const type in EVENTTYPE) {
			...
			for (const subtype in subEventType) {
				this._eventListeners[
					subEventType[subtype as keyof typeof subEventType]
				] = [];
			}
		}
	}
	...
	public emitEvent(type: string | MainEventTypes, event: IEvent): void {
		...
		if (
			this._eventListeners[typeString] &&
		...
		if (typeString.includes("."))
			this.emitEvent(
				typeString.substr(0, typeString.indexOf(".")),
				event,
			);
	}
	...
	public removeListener(token: string): boolean {
		for (const type in EVENTTYPE) {
			...
			for (
				let i = 0;
				i < this._eventListeners[typeLowerCase]!.length;
				i++
			) {
	...
	private convertTypeToString(type: string | MainEventTypes): string {
		...
		if (!typeString || !this._eventListeners[typeString]) {
			this._logger.warn(
				"EventEngine.convertTypeToString: No valid type provided.",
			);
			return "";
		}

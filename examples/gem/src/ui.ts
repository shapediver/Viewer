...
export const updateCustomUi = (
	...
	for (let p in dev) {
		(<any>gemDefinition[<keyof IGemMaterialProperties>p]) =
			dev[<keyof IGemMaterialProperties>p];

		const paramDiv = <HTMLDivElement>(
			parent.querySelector("[name=" + p + "]")
		);
		...
		if (paramType === "slider") {
			...
			(<HTMLOptionElement>(
				inputElement.childNodes[valueIndex]
...
export const createCustomUi = (
	...
	for (let i = 0; i < elements.length; i++) {
		...
		if (menuElement.type === "slider") {
			...
			const dropdownElement: IDropdownElement = <IDropdownElement>(
				menuElement
			);
...
export const updateParameterUi = (
	...
	for (let p in parameters) {
		...
		if (
			...
			inputElement.value = parameters[p];
			(<HTMLOptionElement>(
				inputElement.childNodes[+parameters[p]]
			)).setAttribute("selected", "");
			inputElement.onchange!(new Event("custom"));
		}
	}
...
export const createParameterUi = (
	...
	for (let p in session.parameters) {
		...
		let parameterInputElement: HTMLInputElement | HTMLSelectElement | null =
			null;

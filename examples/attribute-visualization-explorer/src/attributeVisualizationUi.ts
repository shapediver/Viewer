...
export const createAttributeVisualizationUi = (
	...
	Object.values(attributeVisualizationEngine.layers).forEach(
	...
	for (const attribute in attributeVisualizationEngine.overview) {
		const attributeDataCollection =
			attributeVisualizationEngine.overview[attribute];
		for (const attributeData of attributeDataCollection) {
			switch (true) {
				case SdtfPrimitiveTypeGuard.isNumberType(
					attributeData.typeHint,
				):
						...
						onChangeCallback: (value: boolean) => {
							...
							if (value) {
								attributes.push(<INumberAttribute>{
									...
									visualization:
										selectedVisualization[attribute] ||
										ATTRIBUTE_VISUALIZATION.BLUE_GREEN_RED,
					...
					elements.push(<IDropdownElement>{
						...
						value: Object.values(ATTRIBUTE_VISUALIZATION).indexOf(
							ATTRIBUTE_VISUALIZATION.BLUE_GREEN_RED,
						),
						choices: Object.keys(ATTRIBUTE_VISUALIZATION),
						onChangeCallback: (value: number) => {
							console.log(attribute, value);
							selectedVisualization[attribute] = Object.values(
								ATTRIBUTE_VISUALIZATION,
							)[value];
							...
							if (attributeDefinition) {
								(
									attributeDefinition as INumberAttribute
								).visualization =
				...
				case SdtfPrimitiveTypeGuard.isStringType(
					attributeData.typeHint,
				):
					elements.push(<IBooleanElement>{
						type: "boolean",
					...
					elements.push(<IDropdownElement>{
						...
						onChangeCallback: (value: number) => {
							...
							if (attributeDefinition) {
								(
									attributeDefinition as IStringAttribute
								).visualization =
									selectedVisualization[
										attributeDefinition.key
									];

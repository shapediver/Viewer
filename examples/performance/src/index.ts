...
const createChart = (canvasId: string) => {
    ...
    evalData.push([0, 0, 0, 0, 0, sections['init'].duration!, sections['customize'].duration!, sections['finish'].duration!])
    const start = sections['sessionResponse'].start! - performanceEvaluation?.start!;
    evalData.push([start, sections['sessionResponse'].duration!, 0, sections['outputLoading'].duration!, 0, 0, 0, 0])

    let count = 0;
    for (let section in sections!) {
        if (section.startsWith('gltfProcessing')) {

            const section2 = section.replace('gltfProcessing', 'loadGltf');
            labels.push('o' + count)
            count++;
            const start = sections[section2].start! - performanceEvaluation?.start!;

            evalData.push([start, 0, sections[section2].duration!, 0, sections[section].duration! - sections[section2].duration!, 0, 0, 0]);
        }
    }
    ...
    const data = {
        ...
        datasets: [
            ...
            {
                label: 'Http Request (session)',
                backgroundColor: '#ffff00',
                data: []
            },
            {
                label: 'Http Request (gltf)',
                backgroundColor: '#ffff00',
                data: []
            },
            {
                label: 'Process (session loading)',
                backgroundColor: '#ddddff',
                data: []
            },
            {
                label: 'Processing (gltf)',
                backgroundColor: '#0000ff',
                data: []
            },
            {
                label: 'Processing (init)',
                backgroundColor: '#0000ff',
                data: []
            },
            {
                label: 'Process (customize)',
                backgroundColor: '#ddddff',
                data: []
            },
            {
    ...
    charts[canvasId] = new Chart(ctx, {
        ...
        options: {
            plugins: {
                tooltip: {
                    filter: (e: TooltipItem<"bar">, index: number, array: TooltipItem<"bar">[]) => {
                        if(e.dataset.label === '') return false;
                        return e.formattedValue !== '0';
                    }
                },
                legend: {
                    labels: {

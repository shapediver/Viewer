...
(async () => {
    let viewport = await ShapeDiverViewer.createViewport({ canvas: <HTMLCanvasElement>document.getElementById('canvas'), id: 'myViewer' });
    let session = await ShapeDiverViewer.createSession({ 

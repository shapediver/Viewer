...
(async () => {
	...
	const godRaysEffectToken = viewport.postProcessing.addEffect(
		godRaysEffectDefinition,
	);

	const output = session
		.getOutputByName("HorizontalBottom")
		.find((o) => !o.format.includes("material"))!;
	viewport.postProcessing.godRaysEffects[godRaysEffectToken].setLightSource(
		output.node!,
	);
		...
		[
			<IDropdownElement>{
				...
				onChangeCallback: (value: string) => {
					godRaysEffectDefinition.properties!.blendFunction =

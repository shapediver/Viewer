...
import { addListener, createSession, createViewport, EVENTTYPE, ISessionApi, ITreeNode, IViewportApi, MaterialUnlitData, RENDERER_TYPE, sceneTree, SDTFItemData } from "@shapediver/viewer";
...
const onStartCallback = async (data: IStageData) => {
    data.goBackwardDiv.style.visibility = "hidden";

    viewport = await createViewport({ canvas: <HTMLCanvasElement>document.getElementById('canvas'), id: 'myViewer' });
...
const onEndCallback = async (data: IStageData) => {
    ...
    data.goBackwardDiv.style.visibility = "";
};

export const stageAttributeVisualization = new Stage(
    "Attribute Visualization",
    'https://shapediverviewer.s3.amazonaws.com/v3/images/SD_color.png',
    onStartCallback,
    onEndCallback,
    onEndCallback
)
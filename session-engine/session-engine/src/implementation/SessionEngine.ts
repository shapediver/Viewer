...
export class SessionEngine implements ISessionEngine {
		...
		if (
			...
			for (const p in this.parameters) {
				...
				if (
					...
					!(
						this.parameters[p] instanceof FileParameter ||
	...
	public async createModelState(
		...
		try {
					...
					for (const parameterId in this.parameters) {
						...
						if (
							...
							parameterSet[parameterId] = (
								" " +
								this.parameters[parameterId].stringify(
			...
			if (image) {
				promises.push(
					this.processImageInput(image).then((result) => {
	...
	public async customize(
		...
		try {
			...
			if (force === false) {
				...
				for (const parameterId in this.parameters)
					if (
						...
						changes = true;
				if (changes === false) return this.node;
			}
			...
			this._logger.debugLow(
				`Session(${this.id}).customize: Customizing session.`,
			);

			this.addBusyMode(customizationId);

			const eventFileUpload: ITaskEvent = {
				type: TASK_TYPE.SESSION_CUSTOMIZATION,
				id: eventId,
				progress: 0.1,
				data: {sessionId: this.id},
			...
			const cancelResult = this.cancelProcess(
				customizationId,
				eventId,
				TASK_TYPE.SESSION_CUSTOMIZATION,
				1,
				{sessionId: this.id},
			);
			...
			this._logger.info(
				`Session(${this.id}).customize: Customizing session with parameters ${JSON.stringify(this.parameterValues)}.`,
			);

			const eventRequest: ITaskEvent = {
				type: TASK_TYPE.SESSION_CUSTOMIZATION,
				id: eventId,
				progress: 0.1,
				data: {sessionId: this.id},
			...
			const oldOutputVersions =
				this._outputLoader.getCurrentOutputVersions();

			const newNode = await this.customizeInternal(
				() => this.#customizationProcess !== customizationId,
				{
					eventId,
					type: TASK_TYPE.SESSION_CUSTOMIZATION,
			...
			const cancelResult2 = this.cancelProcess(
				customizationId,
				eventId,
				TASK_TYPE.SESSION_CUSTOMIZATION,
				1,
			...
			const eventSceneUpdate: ITaskEvent = {
				type: TASK_TYPE.SESSION_CUSTOMIZATION,
				id: eventId,
				progress: 0.9,
				data: {sessionId: this.id},
			...
			if (waitForViewportUpdate === false) {
				...
				await this.waitForUpdateCallbacks(
					newOutputVersions,
					oldOutputVersions,
					newNode,
					oldNode,
			...
			this._logger.debug(
				`Session(${this.id}).customize: Customization request finished, updating geometry.`,
			);

			// set the session values to the current ones in all parameters
			for (const parameterId in this.parameters)
				(<unknown>this.parameters[parameterId].sessionValue) =
			...
			this.node.excludeViewports = JSON.parse(
				JSON.stringify(this._excludeViewports),
			);

			this.removeBusyMode(customizationId);

			this._logger.debug(
				`Session(${this.id}).customize: Session customized.`,
			);
	...
	public async getModelState(
		...
		try {
			...
			return await this._sdk.modelState.get(id);
		} catch (e) {
			throw this._httpClient.convertError(e);
		}
	}
	...
	public async goBack(): Promise<ITreeNode> {
		if (!this.canGoBack()) {
			this._logger.debug(
				`Session(${this.id}).goBack: Cannot go further back.`,
			);
			return new TreeNode();
		}
		...
		const lastParameterSet =
			this.#parameterHistory[this.#parameterHistory.length - 1];
	...
	public async goForward(): Promise<ITreeNode> {
		if (!this.canGoForward()) {
			this._logger.debug(
				`Session(${this.id}).goForward: Cannot go further forward.`,
			);
			return new TreeNode();
		}
	...
	public async loadCachedOutputsParallel(
		...
		try {
			...
			const node = await this._outputLoader.loadOutputs(
				this._responseDto!.model?.name || "model",
				outputs,
				{},
				taskEventInfo,
				false,
	...
	public async requestExports(
		...
		const treatInternallyAsCustomization =
			loadOutputs === true && this._allowOutputLoading === true;
	...
	public async saveDefaultParameterValues(): Promise<boolean> {
		this._logger.debugLow(
			`Session(${this.id}).saveDefaultParameters: Saving default parameters.`,
		);
		const response = await this.saveDefaultParameters();
		if (response) {
			this._logger.debug(
				`Session(${this.id}).saveDefaultParameters: Saved default parameters.`,
			);
		} else {
			throw new ShapeDiverViewerSessionError(
				`Session(${this.id}).saveDefaultParameters: Could not save default parameters.`,
			);
		}
		return response;
	}
	...
	public async saveUiProperties(
		...
		this._logger.debugLow(
			`Session(${this.id}).saveSessionProperties: Saving session properties.`,
		);
		...
		for (const p in this.parameters) {
			properties[p] = {
				displayname:
					this.parameters[p].displayname !== undefined
						? this.parameters[p].displayname!
						: "",
				hidden:
					this.parameters[p].hidden !== undefined
						? this.parameters[p].hidden
						: false,
				order:
					this.parameters[p].order !== undefined
						? this.parameters[p].order!
						: 0,
				tooltip:
					this.parameters[p].tooltip !== undefined
		...
		for (const e in this.exports) {
			properties[e] = {
				displayname:
					this.exports[e].displayname !== undefined
						? this.exports[e].displayname!
						: "",
				hidden:
					this.exports[e].hidden !== undefined
						? this.exports[e].hidden
						: false,
				order:
					this.exports[e].order !== undefined
						? this.exports[e].order!
						: 0,
				tooltip:
					this.exports[e].tooltip !== undefined
		...
		const responseE =
			Object.values(properties).length !== 0
				? await this.saveExportProperties(properties)
				: true;
		...
		for (const o in this.outputs) {
			properties[o] = {
				displayname:
					this.outputs[o].displayname !== undefined
						? this.outputs[o].displayname!
						: "",
				hidden:
					this.outputs[o].hidden !== undefined
						? this.outputs[o].hidden
						: false,
				order:
					this.outputs[o].order !== undefined
						? this.outputs[o].order!
						: 0,
				tooltip:
					this.outputs[o].tooltip !== undefined
		...
		const responseO =
			Object.values(properties).length !== 0
				? await this.saveOutputProperties(properties)
				: true;
		...
		if (response && responseP && responseO && responseE) {
			this._logger.debug(
				`Session(${this.id}).saveSessionProperties: Saved session properties.`,
			);
		} else {
			this._logger.warn(
				`Session(${this.id}).saveSessionProperties: Could not save session properties.`,
			);
		}
	...
	public async updateOutputs(
		...
		this._logger.debugLow(
			`Session(${this.id}).updateOutputs: Updating Outputs.`,
		);
		...
		this._logger.debug(
			`Session(${this.id}).updateOutputs: Updating outputs finished, updating geometry.`,
		);
		...
		this._logger.debug(
			`Session(${this.id}).updateOutputs: Updated outputs.`,
		);
		...
		if (waitForViewportUpdate) {
			for (const r in this._stateEngine.viewportEngines)
				if (
					...
					this._stateEngine.viewportEngines[r]!.update(
						`SessionEngine(${this.id}).updateOutputs`,
					);
	...
	private cancelProcess(
		...
		if (this.#customizationProcess !== customizationId) {
			...
			this._logger.debug(
				`Session(${this.id}).cancelProcess: The request was was exceeded by another request.`,
			);
			return newNode;
			...
			this._logger.debug(
				`Session(${this.id}).cancelProcess: The session was closed during the request.`,
			);
	...
	private checkAvailability(action?: string, checkForModelId = false) {
		...
		const responseDtoAction = this._responseDto.actions?.find(
			(a) => a.name === action,
		);
	...
	private cleanExportParameters(parameters: {
		...
		for (const parameterIdOrName in parameters) {
			...
			const parameterObject = Object.values(this._parameters).find(
				(p) =>
	...
	private async customizeInternal(
		cancelRequest: () => boolean,
		taskEventInfo: OutputLoaderTaskEventInfo,
	): Promise<ISessionTreeNode> {
		return this.customizeSession(
			this._parameterValues,
			cancelRequest,
			taskEventInfo,
		);
	...
	private async customizeSession(
		...
		try {
			...
			if (loadOutputs === true && this._allowOutputLoading === true) {
				...
				if (parallel === true) {
					...
					return this.loadOutputsParallel(
						responseDto,
						cancelRequest,
						taskEventInfo,
					);
	 ...
	 * @returns
	 */
	private getFileParameterSet(parameters: {[key: string]: unknown}): {
			...
			if (this.parameters[parameterId] instanceof FileParameter) {
				fileParameterSet[parameterId] =
					parameters[parameterId] !== undefined
						...
						: (this.parameters[parameterId] as FileParameter).value;
			}
		}
	...
	private removeBusyMode(busyId: string) {
		for (const r in this._stateEngine.viewportEngines) {
			...
			if (this.#customizationBusyModes.includes(busyId))
				this.#customizationBusyModes.splice(
					this.#customizationBusyModes.indexOf(busyId),
					1,
				);
		}
	}
	...
	private updateResponseDto(
		...
		for (const parameterId in this._responseDto.parameters) {
			...
			switch (true) {
				...
				case this._responseDto.parameters[parameterId].type ===
					...
					this.parameters[parameterId] =
						this.createInteractionParameter(
							this._responseDto.parameters[parameterId],
						);
					break;
				case this._responseDto.parameters[parameterId].type ===
		...
		if (!this.initialized) this.#parameterHistory.push(parameterSet);
	...
	private async waitForUpdateCallbacks(
		...
		if (this._updateCallback)
			await Promise.resolve(this._updateCallback(newNode, oldNode));

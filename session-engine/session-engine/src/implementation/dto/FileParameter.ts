	...
	implements IFileParameter
{
	...
	public async upload(v?: File | Blob | string): Promise<string> {
		...
		if (
			typeof value === "string" &&
			((value.length === 36 && this.#uuidGenerator.validate(value)) ||
				value === "")
		)
			return value;
			...
			if (value.type === "") {
				if (types.length === 0) {
					throw new ShapeDiverViewerSessionError(
						`Parameter(${this.id}).upload: Error uploading FileParameter, provided File has no type and could not be guessed from filename. Has to be ${this.format}.`,
					);
				} else {
					fileType = types;
				}
		...
		endings.forEach(
			(e: string) =>
				(types = types.concat(
					MimeTypeUtils.guessMimeTypeFromFilename(e),
				)),
		);
		...
		if (allowedType === false || type === undefined)
			throw new ShapeDiverViewerSessionError(
				`Parameter(${this.id}).upload: Error uploading FileParameter, type of data (${fileType}) is not a valid type. Has to be ${this.format}.`,
			);

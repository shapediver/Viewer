...
Load-time detection and runtime batching are enabled by default. Unique meshes
are matched with accessor identity and a count/type fingerprint; byte hashing
and baked-transform walks run only when another primitive remains a plausible
match after the metadata and sampled-distance checks. Turning batching back on
scans already-loaded opaque triangle geometry with the same matching rules
(shared primitives and byte-identical copies, material excluding color). Baked
...
`viewport.gpuInstancing` is internal (not part of the public API). Disabling it
uses regular per-mesh rendering for that viewport; load-time detection remains
enabled so viewport creation order does not change the result.
...
1. GPU instancing has not been disabled for the viewport.
...
- `.enabled = true` → scan already-loaded opaque triangle geometry and
  reconvert the current scene into batches
- `.enabled = false` → drop instance groups and reconvert to regular meshes
...
interception. The suite verifies the default-on state and covers duplicated
meshes, shared meshes, baked transforms, per-instance visibility, interaction
highlights, the regular-path fallback (pixel-identical rendering with
instancing disabled), and re-enabling instancing after content is already
loaded.

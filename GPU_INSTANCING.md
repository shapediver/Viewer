...
- `.enabled = true` → retain load-time detection (reference-counted across
  viewports), scan already-loaded opaque triangle geometry, and reconvert the
  current scene into batches
...
interaction highlights, the regular-path fallback (pixel-identical
rendering with instancing disabled), and enabling instancing after content
is already loaded.

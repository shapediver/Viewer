...
export class LightEngine implements ILightEngine {
    ...
    public addDirectionalLight(color: vec3, intensity: number, direction: vec3, castShadow: boolean, id?: string): DirectionalLight {
        const light = new DirectionalLight(color, intensity, direction, castShadow, id);
        this._currentLightScene.addLight(light);
        return light;
    }
    ...
    public setFromSettings(): void {
        ...
        for (let lightSceneId in this._settings.lightScenes.value) {
            ...
            for (let lightId in this._settings.lightScenes.value[lightSceneId].lights) {
                ...
                switch (light.type) {
                    case 'directional':
                        l = new DirectionalLight(colorDecoder(light.properties.color), light.properties.intensity, this._converter.toVec3(light.properties.direction), light.properties.castShadow, lightId);
                        break;
                    case 'hemisphere':

	...
	parse(buffer: ArrayBuffer): RGBE {
		const /* return codes for rgbe routines */
			...
			fgets = function (
				buffer: {
					pos: number;
					subarray: (arg0: any, arg1: any) => Iterable<number>;
					byteLength: number;
				},
			...
			RGBE_ReadHeader = function (buffer: Uint8Array) {
				...
				if (
					(buffer as any).pos >= buffer.byteLength ||
		...
		if (RGBE_RETURN_FAILURE !== rgbe_header_info) {
			...
			if (RGBE_RETURN_FAILURE !== image_rgba_data) {
				...
				switch (this.type) {
					case FloatType:
						numElements =
							(image_rgba_data as Uint8Array).length / 4;

...
export class TextureUnifierInjector {
	...
	public async combineTextures(
		...
		if (!red && !green && !blue) throw new Error("No maps supplied.");
		...
		if (!this._renderer) this.createThreeJsUtils();
		...
		for (const t of textures) {
			if (t) {
				if (width === 0 && height === 0) {
					...
					throw new Error(
						"Maps have different sizes. Combining not supported.",
					);
				}
			}
		}

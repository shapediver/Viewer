...
export class GroundPlaneShadow implements IEnvironmentGeometry {
	...
	constructor(
		...
		this._groundPlaneShadow = new THREE.Mesh(
			new THREE.PlaneGeometry(),
			this._renderingEngine.materialLoader.load(matShadow),
		);
	...
	public changeSceneExtents(
		...
		this._groundPlaneShadow.geometry = new THREE.PlaneGeometry(
			2 * gridExtents,
			2 * gridExtents,
			2,
			2,
		);
		this._groundPlaneShadow.position.set(
			position[0],
			position[1],
			position[2],
		);
	...
	private assignGroundPlaneShadowColor(color: Color) {
		(<THREE.ShadowMaterial>this._groundPlaneShadow.material).opacity =
			this._converter.toAlpha(color);
		(<THREE.ShadowMaterial>this._groundPlaneShadow.material).color =
			this._renderingEngine.createThreeJsColor(color);
		(<THREE.ShadowMaterial>this._groundPlaneShadow.material).needsUpdate =
			true;
	}

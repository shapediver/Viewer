...
export class Grid implements IEnvironmentGeometry {
    ...
    constructor(private readonly _renderingEngine: RenderingEngine, private readonly _parent: SDObject) {
        ...
        (<THREE.LineBasicMaterial>this._grid.material).opacity = typeof this._color == 'string' && this._color.length <= 8 ? 0.15 : this._converter.toAlpha(this._color);
        (<THREE.LineBasicMaterial>this._grid.material).transparent = (<THREE.LineBasicMaterial>this._grid.material).opacity !== 1;
        (<THREE.LineBasicMaterial>this._grid.material).color = this._renderingEngine.createThreeJsColor(this._color);
    ...
    public updatePosition(position: vec3): void {
        if (this._grid) this._grid.position.set(position[0], position[1], position[2]);
    }

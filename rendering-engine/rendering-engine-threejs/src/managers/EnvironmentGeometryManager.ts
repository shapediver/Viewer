...
export class EnvironmentGeometryManager implements IManager {
	...
	public changeSceneExtents(bb: IBox) {
		...
		const {divisions, gridExtents} =
			this.evaluateGridMeasurements(sceneExtents);
		...
		this._grid?.changeSceneExtents(
			vec3.sub(vec3.create(), position, vec3.fromValues(0, 0, eps)),
			divisions,
			gridExtents,
		);
		this._groundPlaneShadow?.changeSceneExtents(
			vec3.sub(vec3.create(), position, vec3.fromValues(0, 0, eps * 3)),
			divisions,
			gridExtents,
		);
		this._contactShadow?.changeSceneExtents(
			vec3.sub(vec3.create(), position, vec3.fromValues(0, 0, eps * 2)),
			divisions,
			gridExtents,
		);
		this._groundPlane?.changeSceneExtents(
			vec3.sub(vec3.create(), position, vec3.fromValues(0, 0, eps * 4)),
			divisions,
			gridExtents,
		);
	}
	...
	public init(): void {
		this._environmentGeometryObject = new SDObject(
			"environmentGeometry",
			"",
		);
		this._renderingEngine.sceneTreeManager.scene.add(
			this._environmentGeometryObject,
		);

		this._contactShadow = new ContactShadow(
		...
		this._groundPlaneShadow = new GroundPlaneShadow(
			this._renderingEngine,
			this._environmentGeometryObject,
		);
	...
	public updateEnvironmentGeometryPosition(): void {
		const bb = new Box(
			this._renderingEngine.sceneTreeManager.boundingBox.min,
			this._renderingEngine.sceneTreeManager.boundingBox.max,
		);
		if (
			(bb.min[0] === 0 &&
				bb.min[1] === 0 &&
		...
		if (!this._initialized) {
			...
			this._groundPlaneShadow?.changeSceneExtents(
				vec3.sub(
					vec3.create(),
					position,
					vec3.fromValues(0, 0, eps * 2),
				),
				divisions,
				gridExtents,
			);
			this._groundPlane?.updatePosition(
				vec3.sub(
					vec3.create(),
					position,
					vec3.fromValues(0, 0, eps * 4),
				),
			);
		}
	}

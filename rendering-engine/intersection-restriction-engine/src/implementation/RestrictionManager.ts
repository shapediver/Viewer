...
export class RestrictionManager implements IRestrictionManager {
	...
	public set showRestrictionVisualization(value: boolean) {
		...
		for (const restriction of Object.values(this.#restrictions)) {
			...
			for (const snapRestriction of Object.values(
				restriction.snapRestrictions,
			)) {
	...
	public close(): void {
		...
		Object.keys(this.#restrictions).forEach((key) =>
			this.removeRestriction(key),
		);
	}

	public getPressedKeys(): string[] {
		return Object.keys(this.#keysPressed).filter(
			(key) => this.#keysPressed[key] === true,
		);
	}

	public getToggleKeys(): string[] {
		return Object.keys(this.#keysToggled).filter(
			(key) => this.#keysToggled[key] === true,
		);
	}
	...
	public rayTrace(
		...
		for (const restrictionId in this.#restrictions) {
			...
			const distanceSquared =
				hit.distanceOriginToClosestIntersectionPointSquared;
		...
		restrictionResults.sort(
			(a, b) =>
				a.distanceOriginToClosestIntersectionPointSquared! -
		...
		if (isDraggingRestriction(metaData)) {
			const {matrix, dragAnchor} = calculateDragMatrix(
				restrictionResult.targetPoint,
				(restrictionResult.restriction as IRestriction).rotation,
				metaData.dragOrigin,
				metaData.dragAnchors,
				restrictionResult.closestIntersectionPoint,
			);
	...
	public removeRestriction(token: string): boolean {
		if (this.#restrictions[token]) {
			Object.values(this.#restrictions[token].snapRestrictions).forEach(
				(r) => r.removeVisualization(),
			);
	...
	private onKeyDown(event: KeyboardEvent): void {
		...
		if (this.#keysToggled[event.key] === undefined)
			this.#keysToggled[event.key] = true;

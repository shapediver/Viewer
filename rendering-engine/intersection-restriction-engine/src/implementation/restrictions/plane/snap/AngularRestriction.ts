import {type ITreeNode, type IViewportApi, sceneTree} from "@shapediver/viewer";
...
export class AngularRestriction
	extends AbstractSnapRestriction
	implements ISnapRestriction
{
	...
	public set active(value: boolean) {
		...
		if (value === false) {
			...
			if (this.#labelNext && this.#activePolarGrids.next)
				this.#labelNext.visible = true;
			if (this.#labelPrevious && this.#activePolarGrids.previous)
	...
	public snap(
		...
		const previousPreviousIndex = this.getPreviousIndex(
			previousIndex,
			positionArray,
		);
		...
		const nextPointFromData = vec3.fromValues(
			positionArray.at(nextIndex * 3)!,
			positionArray.at(nextIndex * 3 + 1)!,
			positionArray.at(nextIndex * 3 + 2)!,
		);
		const nextNextPointFromData = vec3.fromValues(
			positionArray.at(nextNextIndex * 3)!,
			positionArray.at(nextNextIndex * 3 + 1)!,
			positionArray.at(nextNextIndex * 3 + 2)!,
		);
		const previousPointFromData = vec3.fromValues(
			positionArray.at(previousIndex * 3)!,
			positionArray.at(previousIndex * 3 + 1)!,
			positionArray.at(previousIndex * 3 + 2)!,
		);
		const previousPreviousPointFromData = vec3.fromValues(
			positionArray.at(previousPreviousIndex * 3)!,
			positionArray.at(previousPreviousIndex * 3 + 1)!,
			positionArray.at(previousPreviousIndex * 3 + 2)!,
		);

		// project them onto the same plane as the point
		const nextPointProjected = vec3.sub(
			vec3.create(),
			nextPointFromData,
			vec3.scale(
				vec3.create(),
				this.#planeRestriction.normal,
				vec3.dot(
					vec3.sub(vec3.create(), nextPointFromData, point),
					this.#planeRestriction.normal,
				),
			),
		);
		const nextNextPointProjected = vec3.sub(
			vec3.create(),
			nextNextPointFromData,
			vec3.scale(
				vec3.create(),
				this.#planeRestriction.normal,
				vec3.dot(
					vec3.sub(vec3.create(), nextNextPointFromData, point),
					this.#planeRestriction.normal,
				),
			),
		);
		const previousPointProjected = vec3.sub(
			vec3.create(),
			previousPointFromData,
			vec3.scale(
				vec3.create(),
				this.#planeRestriction.normal,
				vec3.dot(
					vec3.sub(vec3.create(), previousPointFromData, point),
					this.#planeRestriction.normal,
				),
			),
		);
		const previousPreviousPointProjected = vec3.sub(
			vec3.create(),
			previousPreviousPointFromData,
			vec3.scale(
				vec3.create(),
				this.#planeRestriction.normal,
				vec3.dot(
		...
		const pointProjected = vec3.transformMat4(
			vec3.create(),
			point,
			this.#planeRestriction.transformationToXYPlaneMatrix,
		);
		vec3.transformMat4(
			nextPointProjected,
			nextPointProjected,
		...
		vec3.transformMat4(
			nextNextPointProjected,
			nextNextPointProjected,
			this.#planeRestriction.transformationToXYPlaneMatrix,
		);
		vec3.transformMat4(
			previousPointProjected,
			previousPointProjected,
			this.#planeRestriction.transformationToXYPlaneMatrix,
		);
		vec3.transformMat4(
			previousPreviousPointProjected,
			previousPreviousPointProjected,
			this.#planeRestriction.transformationToXYPlaneMatrix,
		);
		...
		const {
			angularDifference: angularDifferenceNext,
			crossProduct: crossProductNext,
			closestAngle: closestAngleNext,
		} = this.getAngularDifference(
			{start: nextPointProjected, end: nextNextPointProjected},
			{start: nextPointProjected, end: pointProjected},
		);
		const {
			angularDifference: angularDifferencePrevious,
			crossProduct: crossProductPrevious,
			closestAngle: closestAnglePrevious,
		} = this.getAngularDifference(
			{
		...
		const resultPointNextAngle = vec3.rotateZ(
			vec3.create(),
			pointProjected,
			nextPointProjected,
			crossProductNext[2] < 0
				? -angularDifferenceNext
				: angularDifferenceNext,
		);
		const screenSpaceDistanceCheckNextAngle =
			this.#geometryMathManager.screenSpaceDistanceCheck(
				resultPointNextAngle,
				pointProjected,
				this.#settings.points.size_0! *
					this.#settings.distanceMultiplicationFactor,
			);
		const resultPointPreviousAngle = vec3.rotateZ(
			vec3.create(),
			pointProjected,
			previousPointProjected,
			crossProductPrevious[2] < 0
				? -angularDifferencePrevious
				: angularDifferencePrevious,
		);
		const screenSpaceDistanceCheckPreviousAngle =
			this.#geometryMathManager.screenSpaceDistanceCheck(
				resultPointPreviousAngle,
				pointProjected,
				this.#settings.points.size_0! *
					this.#settings.distanceMultiplicationFactor,
			);
		...
		if (
			screenSpaceDistanceCheckNextAngle.check === false &&
		...
		if (
			...
			const rayDirectionPrevious = vec3.normalize(
				vec3.create(),
				vec3.sub(
					vec3.create(),
					resultPointPreviousAngle,
			...
			if (crossProductLength < 0.001) {
				vec3.transformMat4(
					resultPointNextAngle,
					resultPointNextAngle,
					this.#planeRestriction.transformationFromXYPlaneMatrix,
				);
				return {
					targetPoint: resultPointNextAngle,
					distanceClosestPointToTargetPointSquared: vec3.sqrDist(
						resultPointNextAngle,
						point,
					),
					closestIntersectionPoint: point,
					distanceOriginToClosestIntersectionPointSquared: distance,
					restriction: this.#planeRestriction,
			...
			const intersection = vec3.add(
				vec3.create(),
				nextPointProjected,
				vec3.scale(vec3.create(), rayDirectionNext, tValue),
			);
			this.#labelNext = this.createGrid(
				this.#labelNext,
				nextPointFromData,
				closestAngleNext,
			);
			this.#activePolarGrids.next = true;
			this.#labelPrevious = this.createGrid(
				this.#labelPrevious,
				previousPointFromData,
				closestAnglePrevious,
			);
		...
		if (
			screenSpaceDistanceCheckNextAngle.distanceSquared <
	...
	private createGrid(
		label: CSS2DObject | undefined,
		position: vec3,
		angle: number,
	): CSS2DObject {
		if (label) this._object3D.remove(label);
	...
	private getAngularDifference(
		...
		const lineDirection = vec3.normalize(
			vec3.create(),
			vec3.sub(vec3.create(), line.end, line.start),
		);
		const referenceLineDirection = vec3.normalize(
			vec3.create(),
			vec3.sub(vec3.create(), referenceLine.end, referenceLine.start),
		);

		// calculate the angle between the lineDirection and the referenceLineDirection
		const angleReference = vec3.angle(
			lineDirection,

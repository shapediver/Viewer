...
export interface CameraPlaneRestrictionProperties
...
export class CameraPlaneRestriction
	...
	implements IRestriction
{
	...
	constructor(
		...
		properties: CameraPlaneRestrictionProperties,
	) {
	...
	public rayTrace(
		...
		const cameraDirection = vec3.normalize(
			...
			vec3.sub(
				...
				this.#viewport.camera!.position,
			),
		);
		this.#dragPlane = new Plane().setFromNormalAndCoplanarPoint(
			cameraDirection,
			metaData.startPoint || vec3.create(),
		);

	...
	if (dragAnchors.length > 0) {
		...
		for (let i = 0; i < dragAnchors.length; i++) {
			const matrix = calculateMatrix(
				dragAnchors[i].position,
				dragAnchors[i].rotation || {
		...
		results.sort(
			(a, b) =>
				vec3.distance(a.point, closestPoint!) -
				vec3.distance(b.point, closestPoint!),
		);
		return results[0];
	} else {
		return {
			matrix: calculateMatrix(
				dragOrigin,
				{axis: vec3.fromValues(0, 0, 1), angle: 0},
				snapPoint,
				snapRotation,
			),
			point: dragOrigin,
...
const calculateMatrix = (
	...
	const dragMatrix = mat4.fromRotation(
		mat4.create(),
		dragRotation.angle,
		dragRotation.axis,
	);
	...
	if (!invertedDragMatrix) invertedDragMatrix = mat4.create();

	mat4.multiply(rotationMatrix, rotationMatrix, invertedDragMatrix);

	// apply snap matrix
	const snapMatrix = mat4.fromRotation(
		mat4.create(),
		snapRotation.angle,
		snapRotation.axis,
	);
	...
	const dragPointTransformed = vec3.transformMat4(
		vec3.create(),
		dragPoint,
		rotationMatrix,
	);
	...
	const dragTranslation = vec3.sub(
		vec3.create(),
		snapPoint,
		dragPointTransformed,
	);

	// transformation of the difference
	let invertedRotationMatrix = mat4.invert(mat4.create(), rotationMatrix);
	if (!invertedRotationMatrix) invertedRotationMatrix = mat4.create();

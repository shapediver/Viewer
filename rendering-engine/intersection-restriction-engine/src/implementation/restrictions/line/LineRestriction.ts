	...
	implements IRestriction
{
	...
	constructor(
		...
		id: string,
		settings: IVisualizationSettings,
		properties: LineRestrictionProperties,
	) {
		...
		if (properties.point2Radius !== undefined) {
			this.#point2Restriction = new PointRestriction(
				viewport,
				geometryMathManager,
				parentNode,
				id,
	...
	public isWithinRadius(point: vec3): boolean {
		...
		const squaredRadius1 = this.#point1Restriction?.radius
			? this.#radius > this.#point1Restriction.radius
				? this.#radius * this.#radius
				: this.#point1Restriction.radius *
					this.#point1Restriction.radius
			: this.#radius * this.#radius;
		...
		const squaredRadius2 = this.#point2Restriction?.radius
			? this.#radius > this.#point2Restriction.radius
				? this.#radius * this.#radius
				: this.#point2Restriction.radius *
					this.#point2Restriction.radius
			: this.#radius * this.#radius;
		...
		const projection =
			vec3.dot(vec3.sub(vec3.create(), point, this.#point1), lineDir) /
			lineLengthSquared;
		...
		const closestPointOnLine = vec3.scaleAndAdd(
			vec3.create(),
			this.#point1,
			lineDir,
			clampedProjection,
	...
	public rayTrace(
		...
		const planeNormal = vec3.cross(
			vec3.create(),
			ray.direction,
			this.#dragRay.direction,
		);

		const Na = vec3.normalize(
			vec3.create(),
			vec3.cross(vec3.create(), ray.direction, planeNormal),
		);
		const Nb = vec3.normalize(
			vec3.create(),
			vec3.cross(vec3.create(), this.#dragRay.direction, planeNormal),
		);

		const da =
			vec3.dot(
				vec3.sub(vec3.create(), this.#dragRay.origin, ray.origin),
				Nb,
			) / vec3.dot(ray.direction, Nb);
		const db =
			vec3.dot(
				vec3.sub(vec3.create(), ray.origin, this.#dragRay.origin),
				Na,
			) / vec3.dot(this.#dragRay.direction, Na);
		...
		const result1 = this.#point1Restriction
			? this.#point1Restriction.rayTrace(ray)
			: undefined;
		const result2 = this.#point2Restriction
			? this.#point2Restriction.rayTrace(ray)
			: undefined;
		...
		if (distance < this.#radius * this.#radius) {
			...
			if (distanceOrigin < this.#radius * this.#radius) {
				return {
					...
					distanceOriginToClosestIntersectionPointSquared:
						vec3.sqrDist(ray.origin, pointA),
					targetPoint: pointB,

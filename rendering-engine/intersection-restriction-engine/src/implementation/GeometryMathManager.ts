...
export class GeometryMathManager {
	...
	public checkLineDistances(
		...
		for (let i = 0; i < indicesArrayLines.length; i += 2) {
			...
			distances.push({
				...
				distance: vec3.distance(closestPointOnRay, closestPointOnLine),
			});
		}
	...
	public checkPointDistances(
		...
		for (let i = 0; i < positionArray.length; i += 3) {
			const point = vec3.fromValues(
				positionArray.at(i)!,
				positionArray.at(i + 1)!,
				positionArray.at(i + 2)!,
			);
	...
	public closestPoint(ray: IRay, point: vec3): vec3 {
		...
		const dot = vec3.dot(
			ray.direction,
			vec3.sub(vec3.create(), point, ray.origin),
		);
	...
	public closestPointOnPlane(
		...
		const projectionVector = vec3.scale(
			vec3.create(),
			planeNormal,
			projectionLength,
		);
	 ...
	 * @returns
	 */
	public closestPointsRayLine(
		ray: IRay,
		lineStart: vec3,
		lineEnd: vec3,
	): {closestPointOnRay: vec3; closestPointOnLine: vec3} {
		// direction of line
		const lineDirection = vec3.normalize(
			vec3.create(),
			vec3.subtract(vec3.create(), lineEnd, lineStart),
		);
		...
		if (crossProductLength < 0.0001) {
			...
			const closestPointOnLine = vec3.add(
				...
				vec3.scale(
					...
					lineDirection,
					vec3.dot(
						vec3.subtract(vec3.create(), ray.origin, lineStart),
						lineDirection,
					),
				),
			);
		...
		const closestPointOnRay = vec3.add(
			vec3.create(),
			ray.origin,
			vec3.scale(vec3.create(), ray.direction, tValue),
		);
		...
		if (uValue < 0) {
			...
			closestPointOnLine = vec3.add(
				vec3.create(),
				lineStart,
				vec3.scale(vec3.create(), lineDirection, uValue),
			);
		}
	 ...
	 * @returns
	 */
	public closestPointsRayRay(
		ray1: IRay,
		ray2: IRay,
	): {closestPointOnRay1: vec3; closestPointOnRay2: vec3} {
		...
		if (crossProductLength < 0.0001) {
			...
			const closestPointOnRay2 = vec3.add(
				vec3.create(),
				ray2.origin,
				vec3.scale(
					vec3.create(),
					ray2.direction,
					vec3.dot(
						vec3.subtract(vec3.create(), ray1.origin, ray2.origin),
						ray2.direction,
					),
				),
			);
		...
		const closestPointOnRay1 = vec3.add(
			vec3.create(),
			ray1.origin,
			vec3.scale(vec3.create(), ray1.direction, tValue),
		);
		const closestPointOnRay2 = vec3.add(
			vec3.create(),
			ray2.origin,
			vec3.scale(vec3.create(), ray2.direction, uValue),
		);
	...
	public screenSpaceDistanceCheck(
		point1: vec3,
		point2: vec3,
		threshold: number,
	) {

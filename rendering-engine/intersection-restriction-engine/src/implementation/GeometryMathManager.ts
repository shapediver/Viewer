...
export class GeometryMathManager {
     ...
     * @returns 
     */
    public checkLineDistances(ray: IRay, positionArray: Float32Array, indicesArrayLines?: Uint8Array | null): { index: number[]; distance: number; }[] | undefined {
        ...
        for (let i = 0; i < indicesArrayLines.length; i += 2) {
            ...
            if (this.screenSpaceDistanceCheck(closestPointOnRay, closestPointOnLine, this.#settings.points.size_0! * this.#settings.distanceMultiplicationFactor).check === false) continue;
     ...
     * @returns 
     */
    public checkPointDistances(ray: IRay, positionArray: Float32Array): {
        index: number;
        distance: number;
    }[] | undefined {
        /**
        ...
        for (let i = 0; i < positionArray.length; i += 3) {
            ...
            if (this.screenSpaceDistanceCheck(point, closestPoint, this.#settings.points.size_0! * this.#settings.distanceMultiplicationFactor).check === false) continue;

...
export class CameraInterpolationManager {
	...
	public interpolate(
		...
		this._tween = new this.TweenWrapper(
			parsedOptions,
			newPath.length === 2
				? this.getCameraInterpolation(
						newPath[0],
						newPath[1],
						parsedOptions.coordinates,
					)
				: new CameraMultipleInterpolation(
						this._camera,
						this._cameraControls,
						newPath,
						parsedOptions.interpolation,
					),
			() => {
				this._tween = null;
			},
	...
	private getCameraInterpolation(
		...
		switch (type) {
			case "linear":
				return new CameraLinearInterpolation(
					this._camera,
					this._cameraControls,
					from,
					to,
			...
			case "spherical":
				return new CameraSphericalInterpolation(
					this._camera,
					this._cameraControls,
					from,
					to,
			...
			case "cylindrical":
				return new CameraCylindricalInterpolation(
					this._camera,
					this._cameraControls,
					from,
					to,
			...
			default:
				return new CameraMultipleInterpolation(
					this._camera,
					this._cameraControls,
					[from, to],
	...
	private optionsParser(options: ICameraOptions): {
		duration: number;
		easing: (amount: number) => number;
		coordinates: string;
		interpolation: (v: number[], k: number) => number;
		...
		if (typeof options.easing === "string") {
			...
			const easingFamily =
				TWEEN.Easing[<keyof typeof TWEEN.Easing>keys[0]];
			if (easingFamily) {
				const easingFunction =
					easingFamily[<keyof typeof easingFamily>keys[1]];
		...
		if (typeof options.interpolation === "string") {
			const interpolationFunction =
				TWEEN.Interpolation[
					<keyof typeof TWEEN.Interpolation>options.interpolation
				];
			if (

...
export class CameraCylindricalInterpolation implements ICameraInterpolation {
	...
	constructor(
		...
		this._from_position_heightAdjusted = vec3.fromValues(
			this._from.position[0],
			this._from.position[1],
			this._from.target[2],
		);
		this._r_from = vec3.distance(
			this._from_position_heightAdjusted,
			this._from.target,
		);
		this._dir_from = vec3.normalize(
			vec3.create(),
			vec3.subtract(
				vec3.create(),
				this._from_position_heightAdjusted,
		...
		this._to_position_heightAdjusted = vec3.fromValues(
			this._to.position[0],
			this._to.position[1],
			this._to.target[2],
		);
		this._r_to = vec3.distance(
			this._to_position_heightAdjusted,
			this._to.target,
		);
		this._dir_to = vec3.normalize(
			vec3.create(),
			vec3.subtract(
				vec3.create(),
				this._to_position_heightAdjusted,
		...
		const dot1 = Math.min(
			1,
			Math.max(-1, vec3.dot(this._dir_to, this._dir_from)),
		);
		const dot2 = Math.min(
			1,
			Math.max(-1, vec3.dot(this._lorr, vec3.fromValues(0, 0, 1))),
		);
	...
	public onUpdate(value: {delta: number}): void {
		const t = vec3.add(
			...
			vec3.multiply(
				vec3.create(),
				this._to.target,
				vec3.fromValues(value.delta, value.delta, value.delta),
			),
		);
		const targetOffset = vec3.subtract(
			vec3.create(),
		...
		const scalar =
			this._r_from * (1 - value.delta) + this._r_to * value.delta;
		const p = vec3.add(
			vec3.create(),
			t,
			vec3.multiply(
				vec3.create(),
				dir,
				vec3.fromValues(scalar, scalar, scalar),
			),
		);
		vec3.add(
			p,
			...
			vec3.fromValues(
				0,
				0,
				this._h_from * (1 - value.delta) + this._h_to * value.delta,
			),
		);

...
export class CameraMultipleInterpolation implements ICameraInterpolation {
	...
	public onComplete(value: {delta: number}): void {
		const positionOffset = vec3.subtract(
			vec3.create(),
			vec3.fromValues(
				this._path[this._path.length - 1].position[0],
				this._path[this._path.length - 1].position[1],
				this._path[this._path.length - 1].position[2],
			),
			this._cameraControls.getPositionWithUpdates(),
		);
		this._cameraControls.applyPositionVector(positionOffset);
		const targetOffset = vec3.subtract(
			vec3.create(),
			vec3.fromValues(
				this._path[this._path.length - 1].target[0],
				this._path[this._path.length - 1].target[1],
				this._path[this._path.length - 1].target[2],
			),
			this._cameraControls.getTargetWithUpdates(),
		);
	...
	public onUpdate(value: {delta: number}): void {
		const p: vec3 = vec3.fromValues(
			this._interpolationFunction(this.end.position.x, value.delta),
			this._interpolationFunction(this.end.position.y, value.delta),
			this._interpolationFunction(this.end.position.z, value.delta),
		);
		const positionOffset = vec3.subtract(
			vec3.create(),
			p,
		...
		const t: vec3 = vec3.fromValues(
			this._interpolationFunction(this.end.target.x, value.delta),
			this._interpolationFunction(this.end.target.y, value.delta),
			this._interpolationFunction(this.end.target.z, value.delta),
		);
		const targetOffset = vec3.subtract(
			vec3.create(),
			t,

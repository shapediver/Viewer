...
export class CameraSphericalInterpolation implements ICameraInterpolation {
	...
	constructor(
		...
		this._radius_from = vec3.distance(
			this._from.position,
			this._from.target,
		);
		this._direction_from = vec3.normalize(
			vec3.create(),
			vec3.subtract(
				vec3.create(),
				this._from.position,
		...
		this._direction_to = vec3.normalize(
			vec3.create(),
			vec3.subtract(vec3.create(), this._to.position, this._to.target),
		);

		this._axis = vec3.normalize(
			vec3.create(),
			vec3.cross(vec3.create(), this._direction_to, this._direction_from),
		);
		this._c_angle = -Math.acos(
			Math.min(
				1,
				Math.max(
					-1,
					vec3.dot(this._direction_to, this._direction_from),
				),
			),
		);
	}
	...
	public onUpdate(value: {delta: number}): void {
		...
		const dir = vec3.normalize(
			...
			vec3.transformQuat(
				vec3.create(),
				this._direction_from,
				quat.setAxisAngle(quat.create(), this._axis, angle),
			),
		);

		const scalar =
			this._radius_from * (1 - value.delta) +
			this._radius_to * value.delta;
		const p: vec3 = vec3.add(
			vec3.create(),
			t,
			vec3.multiply(
				vec3.create(),

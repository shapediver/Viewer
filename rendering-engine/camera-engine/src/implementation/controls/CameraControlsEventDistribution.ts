	...
	implements ICameraControlsEventDistribution
{
	...
	public onKey(event: KeyboardEvent): void {
		...
		switch (event.keyCode) {
			case this._controls.input.keys.up:
				...
				this._cameraLogic.pan(
					0,
					this._controls.keyPanSpeed * 0.05,
					true,
					false,
				);
	...
	public onPointerDown(event: PointerEvent): void {
		...
		if (event.isPrimary === true) this._primaryPointerEvent = event;
		else if (event.isPrimary === false) this._secondaryPointerEvent = event;
	...
	public onPointerEnd(event: PointerEvent): void {
		...
		if (event.isPrimary === true) this._primaryPointerEvent = undefined;

...
export class Canvas implements ICanvas {
	...
	constructor(
		...
		private readonly _originalDefinition?: HTMLCanvasElement | string,
		private readonly _canvasElement?: HTMLCanvasElement,
	) {
		if (
			this._originalDefinition &&
			this._originalDefinition instanceof HTMLCanvasElement
		)
			this._originalDefinition = <HTMLCanvasElement>(
				this._originalDefinition.cloneNode(true)
			);

		if (!_canvasElement) {
			this._canvasElement = document.createElement(
				"canvas",

...
export class AnimationEngine implements IManager, IAnimationEngine {
	...
	public update(deltaTime: number): boolean {
		...
		for (let i = 0; i < animations.length; i++) {
			...
			const currentAnimationDeltaTime =
				(animation.animationTime / 1000.0) % animationDuration;

			for (let j = 0; j < animation.tracks.length; j++) {
				const track = animation.tracks[j];
				const id = animation.id + "_" + j;
				if (
					currentAnimationDeltaTime < track.times[0] ||
				...
				for (let k = 1; k < track.times.length; k++) {
					if (
						currentAnimationDeltaTime < track.times[k] &&
						...
						if (track.path === "rotation") {
							...
							if (track.interpolation === "step") {
								quaternion = quat.fromValues(
									track.values[(k - 1) * 4 + 0],
									track.values[(k - 1) * 4 + 1],
									track.values[(k - 1) * 4 + 2],
									track.values[(k - 1) * 4 + 3],
								);
							} else {
							...
							if (track.interpolation === "step") {
								vector = vec3.fromValues(
									track.values[(k - 1) * 3 + 0],
									track.values[(k - 1) * 3 + 1],
							...
							if (pivotMatrix && pivotMatrixInverse) {
								scaleTransformation.matrix = mat4.multiply(
									mat4.create(),
									mat4.multiply(
										mat4.create(),
										pivotMatrix,
	...
	public updateAnimationData(): void {
		...
		for (let i = 0; i < animationArray.length; i++) {
			...
			for (let j = 0; j < names.length; j++)
				if (animationName === names[j]) nameIndices.push(j);

...
export class MultiSelectManager extends AbstractInteractionManager {
    ...
    public onDown(ray: IRay, intersection: IIntersection[]): void {
        ...
        if(this.#nodes.length > 0) {
            if(intersections.length > 0 && !this.#nodes.includes(intersections[0].node)) {
                ...
                console.log('deactivateNode')
                this.deactivateNode(intersections[0]);
            }
    ...
    private activateNode(intersection: IIntersection) {
        ...
        this.viewer.updateNode(intersection.node);
        this.viewer.render();
    }
     ...
     * @param intersection 
     */
    private deactivateNode(intersection: IIntersection) {
        const index = this.#nodes.indexOf(intersection.node);
        if(index === -1) return;
        
        const effectMaterialToken = this.#effectMaterialTokens[index];
        this.#effectMaterialTokens.splice(index, 1);
        if(effectMaterialToken) 
            this.interactionEffectUtils.removeEffectMaterial(intersection.node, effectMaterialToken);
        ...
        this.#eventEngine.emitEvent(EVENTTYPE.INTERACTION.SELECT_OFF, { nodes: this.#nodes, node: intersection.node } as IMultiSelectEvent);

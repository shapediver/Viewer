# `@shapediver/viewer.features.transform-controls`

This is the npm package for the ShapeDiver Viewer TransformControls features. Please have a look at our [help desk section](https://help.shapediver.com/doc/transform-controls) for this feature.
...
```
npm install --save @shapediver/viewer.features.transform-controls
```
...
Once the Gumball has been created, you can listen to the `EVENTTYPE_TRANSFORM_CONTROLS.MATRIX_CHANGED` event which is emitted whenever the movement of the Gumball ends. In this event you get the current nodes and the matrices that are applied to it.
...
const eventListenerToken = addListener(
	...
	(e) => {
		const event =
			e as EventResponseMapping[EVENTTYPE_TRANSFORM_CONTROLS.MATRIX_CHANGED];
...
There are different settings that can be used to tailor the Transform Controls to your needs. You can find them in the table below.
...
| scale               | The scale of the Transform Controls compared to the screen size. | `0.15`  |
| space               | The space in which the Transform Controls operates.              | `local` |

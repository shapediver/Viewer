...
export class DrawingToolsManager implements IDrawingToolsManager {
	...
	public applyConstraints(
		...
		if (constraints.size && pointCount > 0) {
			console.log("[CONSTRAINT] applyConstraints | pointIndex:", pointIndex, "| proposed:", Array.from(proposedPosition).map(v => +v.toFixed(4)), "| sizeConstraints:", JSON.stringify(constraints.size));
			...
			console.log("[CONSTRAINT] peerExtents | min:", Array.from(min).map(v => +v.toFixed(4)), "| max:", Array.from(max).map(v => +v.toFixed(4)));
			for (let i = 0; i < 3; i++) {
		...
		console.log("[CONSTRAINT] result | pointIndex:", pointIndex, "| result:", Array.from(result).map(v => +v.toFixed(4)), "| changed:", result[0] !== proposedPosition[0] || result[1] !== proposedPosition[1] || result[2] !== proposedPosition[2]);
		return result;
	}
	 *
	 * @param index

...
export class GeometryState {
	...
	public canAddPoint(number: number = 1): boolean {
		if (
			this.wasWithinMinimumMaximumPointsRange === true &&
	...
	public checkNumberOfPoints(number?: number): boolean {
		return (
			this.checkMinimumNumberOfPoints(number) &&
			this.checkMaximumNumberOfPoints(number)
		);
	}
	...
	public init() {
		...
		if (geometryProperties.mode !== "points") {
			...
			if (
				...
				this.checkNumberOfPoints(points.length)
			)
				this.#closeLoop = true;
			this.#indicesArrayLines = this.createLineIndices(
				this.#closeLoop ||
					(this.#settings.geometry.close &&
		...
		const updateMaterialVariation = (
			...
			for (const v of variations) {
				(
					this.#geometryDataPoints.material as unknown as {
						[key: string]: unknown;
					}
				)[v as keyof MaterialMultiPointData] = map;
			}
			(
	...
	public updateData(
		...
		if (this.geometryDataLines) {
			...
			if (this.#indicesArrayLines) {
				this.geometryDataLines.primitive.indices = new AttributeData(
					this.#indicesArrayLines,

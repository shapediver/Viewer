	...
	implements ICreationControlCenterViewport
{
	...
	public updateViewports?: (viewportEngines: {
	...
	public applyViewportSettings(
		...
		this.viewportEngines[viewportId].applySettings(
			sections,
			settingsEngine,
		);
	...
	public async closeViewportEngine(id: string): Promise<void> {
		...
		this.#logger.debugLow(
			`CreationControlCenter.closeViewportEngine: Closing viewport ${id}.`,
		);
		if (
			this.#stateEngine.viewportEngines[id]?.initialized.resolved ===
			false
		)
			await new Promise<void>((resolve) => {
				this.#stateEngine.viewportEngines[id]?.initialized.then(() =>
					resolve(),
				);
			});
		...
		this.#logger.debug(
			"CreationControlCenter.closeViewportEngine: Viewport closed.",
		);
		if (this.updateViewports) this.updateViewports(this.viewportEngines);

		this.#eventEngine.emitEvent(EVENTTYPE.VIEWPORT.VIEWPORT_CLOSED, {
			viewportId: id,
		});
	}

	public async createViewportEngine(
		...
		try {
			const eventStart: ITaskEvent = {
				type: TASK_TYPE.VIEWPORT_CREATION,
				id: eventId,
				progress: 0,
			...
			if (this.viewportEngines[viewportEngineId]) {
				...
				this.#logger.warn(
					`CreationControlCenter.createViewport: Viewer with this id (${viewportEngineId}) already exists. Closing initial instance.`,
				);
			...
			if (
				properties.sessionSettingsMode === SESSION_SETTINGS_MODE.MANUAL
			) {
				if (!properties.sessionSettingsId)
					throw new ShapeDiverViewerViewportError(
						"Session with sessionSettingsMode MANUAL needs to have a sessionSettingsId.",
					);
				const sessionSettingsId = properties.sessionSettingsId;
				if (this.#stateEngine.sessionEngines[sessionSettingsId]) {
					await this.assignSettings(
						viewportEngine.id,
						sessionSettingsId,
						true,
					);
				...
				properties.sessionSettingsMode === SESSION_SETTINGS_MODE.FIRST
			) {
				const firstSessionEngine = Object.values(
					this.#stateEngine.sessionEngines,
				).find(
					(sessionEngine) =>
						sessionEngine && sessionEngine.isFirstSession === true,
				);
				if (firstSessionEngine) {
					await this.assignSettings(
						viewportEngine.id,
						firstSessionEngine.id,
						true,
					);
				} else {
					// in createSession
			...
			if (
						...
						(e) => {
							...
							if (event.viewportId === viewportEngine.id) {
								...
								if (boundingBox.isEmpty()) {
									...
									this.showAfterSettingsAssignment(
										viewportEngine,
										viewportEngineId,
									);
								}
				...
				if (properties.visibilitySessionIds) {
					...
					properties.visibilitySessionIds.forEach((sessionId) => {
						if (
							this.#stateEngine.sessionEngines[sessionId] &&
							this.#stateEngine.sessionEngines[sessionId]!
								...
								new Promise<void>((resolve) => {
									if (
										...
										if (
											this.#stateEngine.sessionEngines[
												sessionId
											]!.settingsRegistered.resolved ===
											false
										) {
											this.#stateEngine.sessionEngines[
												sessionId
											]!.settingsRegistered.then(() => {
												resolve();
											});
										} else {
											resolve();
			...
			this.#logger.debug(
				`CreationControlCenter.createViewport: Viewport(${viewportEngineId}) created.`,
			);
	...
	public getViewportSettings(viewportId: string): ISettings {
		...
		if (!viewportEngine)
			throw new ShapeDiverViewerViewportError(
				"Viewport with id " + viewportId + " could not be found.",
			);
	...
	private async assignSettings(
		...
		if (
			...
			this.#stateEngine.sessionEngines[sessionEngineId]!.initialized
				.resolved === true
		) {
			// immediate
	...
	private showAfterSettingsAssignment(
		viewportEngine: RenderingEngineThreeJs,
		viewportEngineId: string,
	): Promise<void> {
		return new Promise<void>((resolve) => {
			if (
				this.#stateEngine.viewportEngines[viewportEngineId]
					?.settingsAssigned.resolved
			) {
				viewportEngine.show = true;
				resolve();
			} else {
